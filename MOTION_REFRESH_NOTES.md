...
The press-impact effect and the prior tap ripple have been removed. Existing same-window links now use a **cinematic rally transition**: a court-light aperture expands from the original link position, a perforated ball and streak cross the screen, and the destination is revealed as the court cover retracts. Hash-only links, external links, downloads, modifier-clicks, and non-link controls retain their existing browser or application behavior. The original href is preserved and used directly after the cover completes.
