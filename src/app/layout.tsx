...
import { minutesSinceManilaMidnight } from "@/lib/skyTime";
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  const settings = await getSiteSettings();
  // Computed server-side so the very first paint already shows the correct
  // Manila-time sky (auto mode) with no flash — the client only needs to
  // reconcile this against a saved Manual preference, if any, on hydration.
  const initialSkyMinutes = minutesSinceManilaMidnight();
  ...
  return (
    <html lang="en">
      ...
      <body className="font-body antialiased">
        <SkyProvider initialMinutes={initialSkyMinutes}>
          <SkyBackdrop />
          <PickleballBackdrop />
