...
export default function AdminDashboard() {
  ...
  const [tab, setTab] = useState<
    "schedule" | "week" | "history" | "feedback" | "accounts" | "pricing" | "hours" | "design" | "music"
  >("schedule");
  ...
  return (
    <div className="min-h-screen bg-court-cream">
            ...
            <div className="relative">
              ...
              {notifOpen && (
                <>
                  ...
                  />
                  <div className="absolute right-0 z-50 mt-2 w-80 max-h-96 overflow-y-auto rounded-court bg-white border-2 border-court-blue/20 shadow-court-lg text-court-ink">
                    <div className="px-4 py-3 border-b border-court-ink/10">
                      <p className="font-display font-600 text-sm">
                        Pending bookings {allPending.length > 0 && `(${allPending.length})`}
                      </p>
                          ...
                          <li key={b.id}>
                            ...
                            >
                              <div className="flex items-center justify-between gap-2">
                                <p className="font-semibold text-sm truncate">{b.customerName}</p>
                              ...
                              <p className="text-xs text-court-ink/60 mt-0.5">
                                {shortDateLabel(b.date)} · {b.startHours.length} slot{b.startHours.length === 1 ? "" : "s"} · ₱{b.grandTotal}
                              </p>
                            </button>
                          </li>
                        ))}
                        ...
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {b.groupId && (
                            <span
                              ...
                              title="This booking's payment also covers a linked reservation on the adjacent day — see Booking history for both."
                            >
                              Multi-day booking
                            </span>
                          )}
      ...
      {rejectingId && (
        // Overlay scrolls itself instead of only centering, so this dialog
        // (which grows with the reason text) is always fully reachable on
        // short screens or with the keyboard open, rather than clipping
        // with no way to scroll down to the buttons.
        <div className="fixed inset-0 z-50 overflow-y-auto overscroll-contain bg-black/40" role="dialog" aria-modal="true">
        <div className="min-h-full flex items-center justify-center px-4 py-8">
          <div className="w-full max-w-md max-h-[85vh] overflow-y-auto rounded-court bg-white border-2 border-court-ink/10 shadow-court p-6">
