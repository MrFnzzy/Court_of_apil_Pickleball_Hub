...
import { startRepeatingAlarm, isAlarmMuted, setAlarmMuted, unlockAudio, playTestAlarm, playPushChime } from "@/lib/adminAlarmSound";
...
import AdminRentalProducts from "@/components/admin/AdminRentalProducts";
...
import AdminCourt360Settings from "@/components/admin/AdminCourt360Settings";
...
type Booking = {
  ...
  /** Every date+hours leg of this reservation (including this row's own),
   * present whenever groupId is set — lets a card for just one date still
   * show the whole multi-day span it belongs to. */
  linkedSlots?: { date: string; startHours: number[] }[];
};
...
// Renders a booking's slots so a 2-day reservation reads as one whole
// booking wherever it's shown — not just whichever single date's row is
// currently on screen. E.g. viewing the Sept 1 leg of a 10PM–1AM overnight
// booking shows "10:00 PM, 11:00 PM (Sep 2 – 12:00 AM–1:00 AM)", and the
// Sept 2 leg shows the same combined line.
function formatBookingSpan(rowDate: string, rowStartHours: number[], linkedSlots?: { date: string; startHours: number[] }[]): string {
  const groups = linkedSlots && linkedSlots.length > 0 ? linkedSlots : [{ date: rowDate, startHours: rowStartHours }];
  const sorted = groups.slice().sort((a, b) => a.date.localeCompare(b.date));
  const rowDateOnly = rowDate.slice(0, 10);
  return sorted
    .map((g) => {
      const hoursLabel = g.startHours.slice().sort((a, b) => a - b).map((h) => labelForSlot(h)).join(", ");
      const gDateOnly = g.date.slice(0, 10);
      if (gDateOnly === rowDateOnly) return hoursLabel;
      const shortDate = new Date(gDateOnly + "T00:00:00").toLocaleDateString("en-PH", { month: "short", day: "numeric" });
      return `(${shortDate} – ${hoursLabel})`;
    })
    .join(", ");
}
...
export default function AdminDashboard() {
  ...
  const [tab, setTab] = useState<
    "schedule" | "week" | "recent" | "history" | "email" | "emailTemplates" | "feedback" | "accounts" | "pricing" | "hours" | "design" | "music" | "court360" | "discounts" | "popup" | "security" | "social" | "faq"
  >("schedule");
  ...
  // Multi-day bookings are stored as two linked rows (see `groupId`) — the
  // row that was clicked only knows its own date's hours, so pull in every
  // leg of the group before opening the edit form. Without this, editing
  // a Sept 1 + Sept 2 booking would silently forget the Sept 2 half the
  // moment it's saved.
  async function openEditBooking(b: Booking) {
    if (!b.groupId) {
      setEditingBooking(b);
      return;
    }
    setEditLoading(true);
    try {
      const res = await fetch(`/api/admin/bookings?groupId=${encodeURIComponent(b.groupId)}`, { cache: "no-store" });
      const data = await res.json();
      const group: Booking[] = Array.isArray(data.bookings) ? data.bookings : [b];
      setEditingBooking({
        ...b,
        groupBookings: group.map((g) => ({ id: g.id, date: g.date, startHours: g.startHours })),
      } as Booking & { groupBookings: { id: string; date: string; startHours: number[] }[] });
    } catch {
      setEditingBooking(b);
    } finally {
  ...
  const loadAllPending = useCallback(async () => {
    try {
      ...
      if (newOnes.length > 0) {
        ...
        activeAlarmRef.current = startRepeatingAlarm(2);
      }
    } catch (err) {
      console.error("Failed to poll pending bookings:", err);
    }
  }, []);
  ...
  // If a real push notification comes in while this tab is open, the
  // service worker (sw.js) can't attach our custom chime to the OS-level
  // notification itself — no browser supports that — so it instead
  // posts a message here and we play the actual paddle-tap + chime +
  // "Dink dink, new booking, don't blink!" cue from this open tab.
  useEffect(() => {
    if (typeof navigator === "undefined" || !("serviceWorker" in navigator)) return;
    function onMessage(event: MessageEvent) {
      if (event.data?.type === "HEIDES_PLAY_CHIME") {
        playPushChime();
      }
    }
  ...
  const loadBookings = useCallback(async () => {
    try {
      ...
      console.error(err);
      setLoadError("Couldn't load bookings — check your connection. Retrying automatically…");
    } finally {
      setLoading(false);
    }
  }, [date, router]);
  ...
  async function updateStatus(id: string, status: string, adminNote?: string) {
    ...
    try {
      ...
      try {
        ...
        // Non-JSON response (e.g. a proxy/500 HTML page) — fall through to
        // the generic error message below instead of throwing here.
      }
      if (!res.ok) {
  ...
  async function removeBooking(id: string) {
    ...
    try {
      ...
      if (!res.ok) {
        let message = "Couldn't remove the booking.";
        try {
          const data = await res.json();
      ...
      console.error("removeBooking failed:", err);
      setActionError("Couldn't reach the server — check your connection and try again.");
    }
  }
  ...
  async function handleLogout() {
    try {
      ...
      // Even if the network request fails, still send the admin back to
      // the login screen rather than leaving them stuck on a dashboard
      // that thinks it's logged in.
    ...
    <div className="min-h-screen bg-gradient-to-br from-court-blue-light/45 via-court-cream to-court-orange-light/25">
            ...
            <div className="relative">
              ...
              {notifOpen && (
                <>
                  ...
                  />
                  <div className="absolute right-0 z-50 mt-2 w-80 max-h-96 overflow-y-auto rounded-court glass-panel glass-scroll text-court-ink">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="relative mb-6 -mx-4 sm:mx-0 px-4 sm:px-0">
          <div className="flex gap-2 overflow-x-auto no-scrollbar snap-x snap-mandatory scroll-px-4 sm:overflow-visible sm:flex-wrap sm:justify-center">
            ...
            <TabButton active={tab === "emailTemplates"} onClick={() => setTab("emailTemplates")}>
              Email templates
            </TabButton>
            ...
            <TabButton active={tab === "court360"} onClick={() => setTab("court360")}>
              360° Court
            </TabButton>
          ...
          <>
            <div className="rounded-court glass-panel p-5 sm:p-6 mb-6">
              <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
                <DatePicker value={date} onChange={setDate} admin />
                <button
                  ...
                  <div key={b.id} className="rounded-court glass-panel p-5">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-court-ink/70 mt-1">
                          {formatBookingSpan(b.date, b.startHours, b.linkedSlots)}
                        </p>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        className="focus-ring rounded-full bg-white text-court-ink/70 border border-court-ink/15 px-4 py-1.5 text-sm font-semibold hover:bg-court-ink/5 disabled:opacity-50"
                      >
                        Edit
                      </button>
      ...
      {rejectingId && (
        ...
        <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto overscroll-contain bg-black/40 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md max-h-[85vh] overflow-y-auto rounded-court glass-panel glass-scroll p-6">
...
type SlotGridItem = { hour: number; status: SlotGridStatus; price: number; booking?: { id: string }; isPast?: boolean };
...
function RescheduleModal({
  ...
  return (
    <ModalPortal>
      <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto overscroll-contain bg-black/40 p-4" role="dialog" aria-modal="true">
        <div className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-court glass-panel glass-scroll p-6">
          ...
          <p className="text-sm text-court-ink/60 mb-4">
            ...
            {formatBookingSpan(booking.date, booking.startHours, booking.linkedSlots)}
          </p>
          {booking.linkedSlots && booking.linkedSlots.length > 1 && (
            <p className="text-xs text-court-orange-dark bg-court-orange/10 border border-court-orange/20 rounded-lg px-3 py-2 mb-4">
              This is one leg of a multi-day booking. Rescheduling here only moves this date — edit the booking instead if you need to change both dates together.
            </p>
          )}
          ...
          {loadingGrid ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5">
              {sortedHours.map((hour) => {
                ...
                const isPastAvailable = slot.status === "available" && !!slot.isPast;
                ...
                else if (isPastAvailable) cls = "bg-gray-100 text-gray-400 border-gray-200 cursor-pointer hover:bg-gray-200";
                ...
                return (
                  <button
                    ...
                    title={isPastAvailable ? "This hour has already passed, but you can still book it manually." : undefined}
...
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <motion.button
      onClick={onClick}
      whileTap={{ scale: 0.94 }}
      className={`focus-ring relative flex-shrink-0 snap-start whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold ${
        active ? "text-white" : "bg-white text-court-ink/70 border border-court-ink/10 hover:border-court-orange/40"
      }`}
    >
      {active && (
        <motion.span
          layoutId="admin-tab-pill"
          className="absolute inset-0 rounded-full bg-court-orange shadow-court"
