...
function WheelBulbs({ count, spinning }: { count: number; spinning: boolean }) {
  ...
  return (
    <div aria-hidden className="absolute pointer-events-none" style={{ inset: -13 }}>
      {bulbs.map((i) => {
...
// ─── Prize legend (what's on the wheel — mirrors the wedge colours exactly) ──
...
function PrizeLegend({
  ...
  return (
    <div className="w-full lg:w-56 lg:flex-shrink-0">
      <p className="text-[11px] font-bold uppercase tracking-wider text-court-ink/40 mb-3 text-center lg:text-left">
        {alreadySpun ? "What was on the wheel" : "What you could win"}
      </p>
      <ul className="flex flex-wrap justify-center gap-2 lg:flex-col lg:justify-start">
        {prizes.map((p, i) => {
          const isWinner = winningIndex != null && i === winningIndex;
          const isDimmed = winningIndex != null && !isWinner;
          return (
            <li
              key={p.id}
              className="flex items-center gap-2.5 rounded-full lg:rounded-xl border px-3.5 py-1.5 text-xs font-semibold transition-all duration-500"
              style={{
                borderColor: isWinner ? "rgba(244,96,54,0.45)" : "rgba(23,58,69,0.1)",
                background: isWinner ? "rgba(244,96,54,0.08)" : "rgba(255,255,255,0.7)",
                opacity: isDimmed ? 0.5 : 1,
                transform: isWinner ? "scale(1.04)" : "scale(1)",
                boxShadow: isWinner ? "0 2px 10px rgba(244,96,54,0.18)" : "none",
              }}
            >
              <span
                className="h-2.5 w-2.5 rounded-full flex-shrink-0"
                style={{ background: colors[i] || p.color, boxShadow: "inset 0 0 0 1.5px rgba(255,255,255,0.6)" }}
              />
              <span className={isWinner ? "text-court-orange-dark" : "text-court-ink/70"}>
                {p.label}
              </span>
...
export default function SpinWheelPage({ params }: { params: { token: string } }) {
  ...
  return (
    <div className="min-h-screen bg-court-cream flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 w-full mx-auto px-4 sm:px-6 py-10 relative">

        {/* ── Loading ── */}
        {loading ? (
          <div className="max-w-xl mx-auto rounded-court bg-white border-2 border-court-ink/10 shadow-court p-8 text-center">
          ...
          <div className="max-w-xl mx-auto rounded-court bg-white border-2 border-court-ink/10 shadow-court p-8 text-center wizard-step">
          ...
          <div className="max-w-4xl mx-auto">
            {isTest && (
              <div className="mb-5 rounded-full bg-court-ink text-white text-xs font-bold uppercase tracking-wide px-4 py-1.5 text-center wizard-step mx-auto w-fit">
            ...
            {/* ── Wheel + legend: side-by-side once there's room, stacked and
                 centred on small screens so the layout always feels balanced
                 rather than a narrow column adrift in empty space. ── */}
            <div className="flex flex-col items-center gap-8 lg:flex-row lg:items-center lg:justify-center lg:gap-14">
              ...
              <div className="flex flex-col items-center">
                ...
                <div
                  ...
                  style={{ width: "clamp(260px, 46vw, 380px)", height: "clamp(260px, 46vw, 380px)" }}
                >
              ...
              {/* ── Legend: balances the composition on wider screens and
                   doubles as an at-a-glance summary of the wheel's prizes ── */}
              <PrizeLegend
                prizes={prizes}
                colors={harmonizedColors}
                winningIndex={winningIndex}
            ...
            {showResultCard && result && (
              <div
                className="max-w-xl mx-auto mt-8 rounded-court shadow-court-lg p-6 sm:p-8 text-center wizard-step relative overflow-hidden"
                style={{
