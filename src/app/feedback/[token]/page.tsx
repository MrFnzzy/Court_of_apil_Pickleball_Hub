...
const RATING_QUESTIONS: { key: RatingKey; label: string; hint: string }[] = [
  ...
  { key: "serviceRating", label: "Staff & service", hint: "How helpful and friendly was the team?" },
  { key: "valueRating", label: "Value for money", hint: "Did the price feel fair for what you got?" },
];

type RatingKey = "overallRating" | "venueRating" | "serviceRating" | "valueRating";
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="rounded-court bg-white border-2 border-court-ink/10 shadow-court p-5 sm:p-6 space-y-6">
              ...
              {!allRated && (
                <p className="text-center text-xs text-court-ink/40">Please rate all four categories to submit.</p>
              )}
            </div>
          </>
        )}
