...
export async function GET(_req: NextRequest, { params }: { params: { token: string } }) {
  const invite = await prisma.spinInvite.findUnique({
    ...
    include: {
      ...
      discount: { select: { code: true, percentage: true, endDate: true } },
    },
  });
  ...
  if (invite.revokedAt) {
    return NextResponse.json({ error: "This spin invite has been disabled." }, { status: 410 });
  }
  if (!invite.spunAt && invite.expiresAt && invite.expiresAt.getTime() < Date.now()) {
    return NextResponse.json({ error: "This spin invite has expired." }, { status: 410 });
  }
  ...
  return NextResponse.json({
    ...
    result: invite.spunAt
      ? {
          ...
          discountExpiresAt: invite.discount?.endDate ? invite.discount.endDate.toISOString() : null,
        }
      : null,
  });
}
...
export async function POST(_req: NextRequest, { params }: { params: { token: string } }) {
  ...
  const claim = await prisma.spinInvite.updateMany({
    where: { id: invite.id, spunAt: null, revokedAt: null },
  ...
  const totalWeight = prizes.reduce((sum: number, p: { weight: number }) => sum + p.weight, 0);
  ...
  if (landed.percentage > 0) {
    ...
    const now = new Date();
    // Null validityDays means the admin set this prize to never expire.
    const expires = landed.validityDays != null ? new Date(now.getTime() + landed.validityDays * 24 * 60 * 60 * 1000) : null;
  ...
  return NextResponse.json({
    ...
    discountExpiresAt: discountExpiresAt ? discountExpiresAt.toISOString() : null,
  });
}
