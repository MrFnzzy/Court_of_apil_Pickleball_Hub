...
// requests also get a `linkedBookings` field on any booking that's part of
// a multi-day group (see groupId on the Booking model) — every other date
// + hours it was booked together with — so the dashboard can flag it as a
// multi-day booking and show all its legs without a second lookup.
export async function GET(req: NextRequest) {
  try {
    ...
    // attach a compact summary of every *other* date/hours in the group onto
    // each booking, so the dashboard's popover can flag it as a multi-day
    // booking and show where the rest of it lives (any number of linked
    // dates, not just a single other one).
    ...
    if (isAdmin) {
      ...
      if (groupIds.length > 0) {
        ...
        for (const s of slots as (typeof slots)[number][]) {
          ...
          if (linkedByBookingId.has(b.id)) continue;
          const siblings = byGroup.get(b.groupId) || [];
          const others = siblings
            .filter((r: (typeof groupRows)[number]) => r.id !== b.id)
            .sort((a: (typeof groupRows)[number], c: (typeof groupRows)[number]) => a.date.getTime() - c.date.getTime())
            .map((r: (typeof groupRows)[number]) => ({ date: r.date.toISOString().slice(0, 10), startHours: r.startHours }));
          if (others.length > 0) {
            linkedByBookingId.set(b.id, others);
          }
        }
      }
    }
      ...
      return {
        ...
        booking:
          isAdmin && slot
            ? {
                ...(slot.booking as any),
                linkedBookings: linkedByBookingId.get((slot.booking as any).id) || [],
              }
            : undefined,
