...
import { sendDiscountExpiryNotifications } from "@/lib/discountExpiryEmail";
import { sendSpinInviteExpiryReminders } from "@/lib/spinInviteExpiryEmail";
...
// Cap on how many distinct dates a single checkout/booking can combine.
// Generous enough for "book a handful of scattered days at once" while
// still bounding the size of one transaction/email/etc.
...
export async function POST(req: NextRequest) {
  try {
    ...
    await sendDiscountExpiryNotifications();
    await sendSpinInviteExpiryReminders();
    ...
    const {
      ...
      contactNumber,
      email,
      // Multi-day shape: selections = [{ date, hours }, ...] — any number of
      // distinct dates (up to MAX_BOOKING_DATES), not just a pair, booked
      // together in one transaction and linked by a shared groupId. Legacy
      // single-day shape (date + hours) is still accepted below and
      // normalized into `selections`.
      ...
      promoCode,
      // reCAPTCHA v2 token from the checkbox (+ puzzle, if triggered) the
      // customer completed on the payment step — see verifyRecaptcha() for
    ...
    if (selections.length === 0 || selections.length > MAX_BOOKING_DATES) {
      return NextResponse.json(
        selections.length > MAX_BOOKING_DATES
          ? { error: `Please combine slots from at most ${MAX_BOOKING_DATES} dates in one booking.` }
          : { error: "Please select at least one valid time slot." },
        { status: 400 }
      );
    }
    ...
    // Reject any selection that falls inside an admin-blocked month (see
    // blockedMonths on SiteSettings) — ScheduleGrid already hides these
    // dates from the customer entirely, so reaching this check normally
    // means a stale page or a direct API call, not a customer following
    // the normal flow.
    const siteSettingsForBlock = await getSiteSettings();
    if (siteSettingsForBlock.blockedMonths.length > 0) {
      for (const sel of selections) {
        if (siteSettingsForBlock.blockedMonths.includes(sel.date.slice(0, 7))) {
          return NextResponse.json({ error: siteSettingsForBlock.blockedMonthsMessage }, { status: 400 });
        }
      }
    }
    // A booking may span a single day, or combine slots from several
    // different dates (any dates, not just consecutive ones — e.g. Sept 1
    // and Sept 10 in the same checkout) in one transaction. Just make sure
    // no date is duplicated across the selections (the client always
    // groups by date, so this should only trip on a malformed request).
    {
      const dates = selections.map((s) => s.date);
      if (new Set(dates).size !== dates.length) {
        return NextResponse.json(
          { error: "Please select each date only once." },
          { status: 400 }
        );
      }
    }
    ...
    const bookings = await prisma.$transaction(async (tx: PrismaNS.TransactionClient) => {
      // One reference code for the whole checkout — every row created below
      // (whether it's a single day or a two-day combo booking) shares it, so
      // the customer only ever has one code to remember per booking made.
    ...
    return NextResponse.json({ success: true, bookingIds: bookings.map((b: { id: string }) => b.id), bookingRef: bookings[0].bookingRef });
  } catch (err: any) {
