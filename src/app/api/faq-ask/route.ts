...
import { getActiveFaqs, type Faq } from "@/lib/faq";
...
function localMatch(question: string, faqs: Faq[]): { text: string; matched: boolean } {
  const qWords = new Set(tokenize(question));
  if (qWords.size === 0 || faqs.length === 0) return { text: FALLBACK_MESSAGE, matched: false };

  let best = { score: 0, entry: faqs[0] };
  ...
  const text = best.entry.link ? `${best.entry.answer}\n${best.entry.linkLabel ?? best.entry.link}` : best.entry.answer;
  return { text, matched: true };
}

async function aiMatch(question: string, apiKey: string, faqs: Faq[]): Promise<{ text: string; matched: boolean }> {
  const faqBlock = faqs
    .map((e, i) => `${i + 1}. Q: ${e.question}\n   A: ${e.answer}${e.link ? ` (${e.linkLabel ?? e.link})` : ""}`)
    .join("\n");
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (apiKey) {
      try {
        const result = await aiMatch(question, apiKey, faqs);
    ...
    const result = localMatch(question, faqs);
    return NextResponse.json({ answer: result.text, matched: result.matched });
  } catch {
