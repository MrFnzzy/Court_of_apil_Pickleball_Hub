...
async function aiMatch(question: string, apiKey: string): Promise<{ text: string; matched: boolean }> {
  ...
  const system = `You are the FAQ assistant on a pickleball court booking website. Answer ONLY using the FAQ list below — never invent policies, prices, or details that aren't in it. Keep answers to 1-3 short sentences, friendly and direct, no markdown formatting. If the question isn't covered by the FAQ (even if related to pickleball generally), don't guess — respond with exactly: NOT_COVERED
