  ...
  try {
    const jsonResponse = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayloadRaw) => {
        ...
        // uploaded by a signed-in admin — unlike proof-of-payment images,
        // which customers upload while booking.
        if ((isMusic || isPopupVideo) && !(await isAdminAuthed())) {
          throw new Error("Unauthorized.");
        }

        const maximumSizeInBytes = isPopupVideo ? 100 * 1024 * 1024 : isMusic ? 25 * 1024 * 1024 : 8 * 1024 * 1024;
