...
// Cap on how many distinct dates a single manual booking can combine —
// same generous limit as the customer-facing checkout.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const {
      ...
      // distinct dates (up to MAX_BOOKING_DATES, don't need to be
      // consecutive) created together and linked with a shared groupId,
      // same as the customer-facing flow. Legacy single-day shape
      // (date + hours) is still accepted and normalized into `selections`.
    ...
    if (!customerName || selections.length === 0 || selections.length > MAX_BOOKING_DATES) {
    ...
    if (notifyCustomer && email && status === "CONFIRMED") {
      try {
        // A 2-day manual booking creates two linked rows (see groupId
        // above) — send ONE email covering every date, not one per row.
        await sendConfirmationEmail({
          email: booking.email,
          customerName: booking.customerName,
          dates: bookings.map((b) => ({ date: b.date, startHours: b.startHours })),
          courtTotal: bookings.reduce((s, b) => s + b.courtTotal, 0),
          rentalTotal: bookings.reduce((s, b) => s + b.rentalTotal, 0),
          ballTotal: bookings.reduce((s, b) => s + b.ballTotal, 0),
          grandTotal: bookings.reduce((s, b) => s + b.grandTotal, 0),
          paddleCount: bookings.reduce((s, b) => s + b.paddleCount, 0),
