...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (mode === "edit") {
    const {
      ...
      // Multi-day shape (same as manual-booking creation): up to two
      // { date, hours } groups covering the whole reservation. The legacy
      // single date/hours shape (still sent by any older client) is
      // normalized into this below.
    ...
    if (selections.length === 0 || selections.length > 2) {
      return NextResponse.json({ error: "Select at least one time slot on the schedule." }, { status: 400 });
    }
