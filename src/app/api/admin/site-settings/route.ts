...
export async function PATCH(req: NextRequest) {
  try {
    ...
    for (const field of EDITABLE_SITE_SETTINGS_FIELDS) {
      ...
      if (field === "blockedMonths") {
        const normalized = normalizeBlockedMonths(value);
        if (!normalized) {
          return NextResponse.json({ error: "blockedMonths must be an array of \"YYYY-MM\" strings." }, { status: 400 });
        }
        (data as any)[field] = normalized;
        continue;
      }
