...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const discount = await prisma.discount.findUnique({ where: { id: params.id }, select: { id: true } });
  if (!discount) return NextResponse.json({ error: "Not found." }, { status: 404 });

  // Deleting a discount is safe even if it's been redeemed: past bookings
  // already have their own discountAmount/discountPercent snapshotted at
  // checkout time, and Booking.discountId / SpinInvite.discountId are set
  // to SetNull on delete — so historical bookings and receipts are
  // unaffected. Only the DiscountRedemption audit rows for this code are
  // removed (cascade), which just means those redemptions no longer count
  // against a promo code that no longer exists.
