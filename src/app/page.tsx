...
export default async function HomePage() {
  ...
  const tickerItems = parseBullets(settings.tickerItems);
      ...
      <main>
        ...
        <div className="playbook-ticker" aria-label="Pickleball highlights">
          <div className="playbook-ticker__track">
            {[...tickerItems, ...tickerItems].map((item, index) => <span className="playbook-ticker__item" key={`${item}-${index}`}><span className="playbook-ticker__dot" />{item}</span>)}
          </div>
        </div>
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <ReservationPreview
            court360Url={settings.court360Url}
            eyebrow={settings.reservationPreviewEyebrow}
            heading={settings.reservationPreviewHeading}
            subtext={settings.reservationPreviewSubtext}
          />
        </section>
        </Reveal>
        ...
        <section className="relative py-6">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <div className="glass-panel rounded-court p-6 sm:p-10 grid md:grid-cols-2 gap-10 items-start">
              ...
              <div>
                <h3 className="font-display font-600 text-lg text-court-ink mb-3">{settings.locationHeading}</h3>
                <MapToggle />
              </div>
            </div>
          </div>
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <div className="motion-grid">
            <div className="motion-grid__feature motion-grid__card fx-tilt glass-panel rounded-court p-6 sm:p-8 text-court-ink">
              <span className="text-xs font-bold uppercase tracking-[.22em] text-court-orange-dark">{settings.rallyEyebrow}</span>
              <h2 className="font-display font-700 text-3xl mt-3">{settings.rallyHeading}</h2>
              <p className="mt-3 text-court-ink/70 leading-relaxed">{settings.rallyText}</p>
              <div className="mt-6 flex items-center gap-2 text-sm font-semibold text-court-blue-dark"><span className="h-2 w-2 rounded-full bg-court-orange animate-pulse" />{settings.rallyReadyText}</div>
            </div>
            <div className="motion-grid__wide grid sm:grid-cols-3 gap-4">
              <div className="motion-grid__card fx-tilt glass-panel rounded-court p-5 text-court-ink"><p className="text-4xl font-display font-700 text-court-orange">{settings.statCard1Value}</p><p className="mt-2 text-sm font-semibold">{settings.statCard1Label}</p><p className="mt-1 text-xs text-court-ink/60">{settings.statCard1Desc}</p></div>
              <div className="motion-grid__card fx-tilt glass-panel rounded-court p-5 text-court-ink"><p className="text-4xl font-display font-700 text-court-blue-dark">{settings.statCard2Value}</p><p className="mt-2 text-sm font-semibold">{settings.statCard2Label}</p><p className="mt-1 text-xs text-court-ink/60">{settings.statCard2Desc}</p></div>
              <div className="motion-grid__card fx-tilt glass-panel rounded-court p-5 text-court-ink"><p className="text-4xl font-display font-700 text-court-orange-dark">{settings.statCard3Value}</p><p className="mt-2 text-sm font-semibold">{settings.statCard3Label}</p><p className="mt-1 text-xs text-court-ink/60">{settings.statCard3Desc}</p></div>
            </div>
          </div>
        </section>
        </Reveal>
        ...
        <Reveal>
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <h2 className="font-display font-700 text-2xl text-white mb-6">{settings.paddleSectionHeading}</h2>
          <RentalProductGrid products={paddleProducts} />
          <p className="text-xs text-white/60 mt-3">{settings.rentalNoteText}</p>
        </section>
        </Reveal>
        ...
        <Reveal>
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          <h2 className="font-display font-700 text-2xl text-white mb-6">{settings.ballSectionHeading}</h2>
        ...
        <section id="pricing" className="relative bg-court-ink text-white overflow-hidden">
          ...
          <div className="relative z-[1] max-w-6xl mx-auto px-4 sm:px-6 py-14">
            <h2 className="font-display font-700 text-2xl mb-6">{settings.courtRatesHeading}</h2>
            <div className="grid sm:grid-cols-3 gap-5">
              <div className="glass-panel-dark rounded-court p-6 transition-colors">
                <p className="text-xs uppercase tracking-widest text-court-blue font-bold mb-2">{settings.weekdayDayLabel}</p>
                <p className="text-lg font-display font-600">{settings.weekdayDayTime}</p>
              ...
              <div className="glass-panel-dark rounded-court p-6 transition-colors">
                <p className="text-xs uppercase tracking-widest text-court-orange-light font-bold mb-2">{settings.weekdayNightLabel}</p>
                <p className="text-lg font-display font-600">{settings.weekdayNightTime}</p>
              ...
              <div className="glass-panel-dark rounded-court p-6 transition-colors">
                <p className="text-xs uppercase tracking-widest text-white font-bold mb-2">{settings.weekendLabel}</p>
                <p className="text-lg font-display font-600">{settings.weekendTime}</p>
        ...
        <section id="schedule" className="max-w-6xl mx-auto px-4 sm:px-6 py-14">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
            <h2 className="font-display font-700 text-2xl text-white">{settings.scheduleSectionHeading}</h2>
            <Link href="/book" className="focus-ring text-sm font-semibold text-court-orange-light hover:underline">
              {settings.scheduleBookLinkText}
            </Link>
          </div>
        ...
        <section id="faq" className="max-w-4xl mx-auto px-4 sm:px-6 py-14">
          <div className="text-center mb-8">
            ...
            <h2 className="font-display font-700 text-2xl sm:text-3xl text-white">{settings.faqHeading}</h2>
