...
export default function BookPage() {
  ...
  // Status of the automatic "read the receipt" pass triggered when a proof
  // of payment screenshot is chosen (see handleFileChange below).
  const [scanningReceipt, setScanningReceipt] = useState(false);
  const [receiptScanNote, setReceiptScanNote] = useState<string | null>(null);
  ...
  // Fires when a proof-of-payment screenshot is chosen. Kicks off an
  // automatic read of the reference number and amount via /api/receipt-scan
  // and pre-fills those two fields — the customer still sees and can edit
  // whatever gets filled in before submitting. Fails silently (falls back to
  // manual entry) if scanning isn't configured or the read isn't confident.
  async function handleFileChange(f: File | null) {
    setFile(f);
    setReceiptScanNote(null);
    ...
    try {
      ...
      if (typeof data.amount === "number" && data.amount > 0) {
        setAmountSent(String(Math.round(data.amount)));
        filledSomething = true;
      }
      setReceiptScanNote(
        filledSomething
          ? "Auto-filled from your screenshot — please double-check these before submitting."
          : "Couldn't read the details from that screenshot — please fill them in below."
      );
    } catch {
      // Silent — the customer can always type these in manually.
    } finally {
            ...
            <section key="step1" className="wizard-step rounded-court bg-white border-2 border-court-blue/20 shadow-court p-5 sm:p-6">
              ...
              {todayFullNotice && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                  <div className="relative w-full max-w-sm rounded-court bg-white border-2 border-court-blue/20 shadow-court-lg p-6 text-center">
                    <button
                      type="button"
            ...
            <section key="step4" className="wizard-step rounded-court bg-white border-2 border-court-blue/20 shadow-court p-5 sm:p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4 mt-4">
                ...
                <Field label="Proof of payment (screenshot)" full>
                  ...
                  />
                  {scanningReceipt && (
                    <p className="mt-1.5 flex items-center gap-1.5 text-xs text-court-ink/50">
                  ...
                  {!scanningReceipt && receiptScanNote && (
                    <p className="mt-1.5 text-xs text-court-blue-dark">{receiptScanNote}</p>
                  )}
                </Field>
              </div>
