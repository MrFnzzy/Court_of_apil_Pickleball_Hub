...
export default function BookPage() {
  ...
  const step4Done = !!paymentMethod && referenceNumber.trim().length > 0 && !!file && noRefundAck;
  ...
  // Toggling a slot keeps the cart valid for the two combinations we
  // support: (a) any single day on its own, or (b) exactly "today" +
  // "tomorrow" together. Switching between those two combo days keeps both
  // sets of picks; jumping to a third, unrelated day starts a fresh
  // single-day cart instead of trying to combine three different dates.
  function toggleHour(hour: number) {
    setCart((prev) => {
      ...
      const isComboDate = date === today || date === tomorrow;
      const kept = isComboDate
        ? prev.filter((c) => c.date === today || c.date === tomorrow)
        : prev.filter((c) => c.date === date);
      return [...kept, { date, hour }];
    });
  }
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section key="step1" className="wizard-step rounded-court bg-white border-2 border-court-blue/20 shadow-court p-5 sm:p-6">
              ...
              <p className="text-xs text-court-ink/50 -mt-2 mb-4">
                Tip: you can pick hours on both today and tomorrow and reserve them together in one booking —
                great for late-night games that cross midnight.
              </p>

              {todayFullNotice && (
                <div className="mb-4 flex items-start gap-2 rounded-xl border-2 border-court-blue-dark/30 bg-court-blue-light/30 text-court-ink px-4 py-3 text-sm font-medium">
                  <svg className="h-4 w-4 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                    <circle cx="12" cy="12" r="9" />
                    <path d="M12 8v5M12 16h.01" strokeLinecap="round" />
                  </svg>
                  <span className="flex-1">Today&apos;s courts are full. Plan your next game by booking a slot tomorrow.</span>
                  <button
                    type="button"
                    onClick={() => setTodayFullNotice(false)}
                    className="focus-ring text-court-ink/40 hover:text-court-ink/70"
                    aria-label="Dismiss"
                  >
                    ✕
                  </button>
            ...
            <section key="step4" className="wizard-step rounded-court bg-white border-2 border-court-blue/20 shadow-court p-5 sm:p-6">
              ...
              <div className="mb-4 rounded-xl border-2 border-red-300 bg-red-50 text-red-700 px-4 py-3 text-sm font-semibold">
                NO REFUND — all payments are final. If your plans change, you may reschedule your booking
                (subject to availability); refunds are not offered.
              </div>
