...
export default function BookPage() {
  ...
  // Promo success animation: anchored to the real promo box + sidebar total
  // elements, so it plays exactly where the P470 reference points at.
  const promoBoxRef = useRef<HTMLDivElement | null>(null);
  const sidebarTotalRef = useRef<HTMLSpanElement | null>(null);
  const [promoAnim, setPromoAnim] = useState<{ runId: number; from: string; to: string } | null>(null);
  const promoAnimatingRef = useRef(false);
  ...
  async function applyPromo() {
    ...
    if (!raw) return;
    // Ignore repeated Apply clicks while a success animation is still
    // playing, rather than queuing a second one on top of it.
    if (promoAnimatingRef.current) return;
    ...
    try {
      ...
      if (!data.valid) {
        ...
        const prevTotal = grandTotal; // total before this discount is applied
        const nextTotal = subtotal - data.discountAmount;
        setAppliedPromo({ code: data.code, percentage: data.percentage, discountAmount: data.discountAmount });
        setPromoCodeInput(data.code);
        promoAnimatingRef.current = true;
        setPromoAnim({ runId: Date.now(), from: `₱${prevTotal}`, to: `₱${nextTotal}` });
      }
    } catch {
  ...
  function removePromo() {
    ...
    setPromoCodeInput("");
    setPromoError(null);
    promoAnimatingRef.current = false;
    setPromoAnim(null);
  }
    ...
    <>
        ...
        <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <section key="step4" className="wizard-step rounded-court bg-white border-2 border-court-blue/20 shadow-court p-5 sm:p-6">
              ...
              {/* Promo code */}
              <div ref={promoBoxRef} className="rounded-xl border-2 border-court-blue/20 bg-court-blue-light/10 p-4">
                <p className="text-sm font-semibold text-court-ink mb-2">Have a promo code?</p>
                {appliedPromo ? (
                  <div className="flex items-center gap-3">
                    <div className="flex-1 flex items-center gap-2 rounded-xl border-2 border-green-400 bg-green-50 px-3 py-2">
                      <svg className="h-4 w-4 text-green-600 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                        <path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                      <span className="font-mono font-bold text-green-800 text-sm tracking-wide">{appliedPromo.code}</span>
                      <span className="text-green-700 text-xs">({appliedPromo.percentage}% off — saving ₱{appliedPromo.discountAmount})</span>
                    </div>
                    <button
                      type="button"
                      onClick={removePromo}
                      className="text-xs font-semibold text-red-500 hover:underline focus-ring"
                    >
                      Remove
                    </button>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={applyPromo}
                      disabled={promoChecking || !promoCodeInput.trim() || !!promoAnim}
                      className="focus-ring rounded-xl bg-court-blue-dark text-white px-4 py-2 text-sm font-semibold hover:brightness-95 disabled:opacity-50"
                    >
            ...
            <div className="wizard-step sticky top-6 rounded-court bg-white border-2 border-court-blue/20 shadow-court-lg p-5 sm:p-6 overflow-hidden" style={{ animationDelay: "140ms" }}>
              ...
              <div className="border-t border-court-ink/10 pt-3 space-y-1 text-sm">
                ...
                <div className="flex justify-between items-baseline font-display font-700 text-court-ink text-base pt-2">
                  <span>Total</span>
                  <span ref={sidebarTotalRef} key={grandTotal} className="total-pulse text-xl text-court-orange">₱{grandTotal}</span>
                </div>
              </div>
      ...
      {promoAnim && (
        <PromoSuccessAnimation
          runId={promoAnim.runId}
          originRef={promoBoxRef}
          targetRef={sidebarTotalRef}
          fromLabel={promoAnim.from}
          toLabel={promoAnim.to}
          onComplete={() => {
            promoAnimatingRef.current = false;
            setPromoAnim(null);
          }}
