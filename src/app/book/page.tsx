...
export default function BookPage() {
  ...
  async function applyPromoCode() {
    ...
    if (!trimmed) return setPromoError("Please enter a promo code.");
    if (!email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return setPromoError("Please fill in your email in Step 3 first so we can validate your code.");
    }
        ...
        <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <section key="step4" className="wizard-step rounded-court bg-white border-2 border-court-blue/20 shadow-court p-5 sm:p-6">
              ...
              {/* Promo Code */}
              <div className="mt-4 pt-4 border-t border-court-ink/10">
                <p className="text-sm font-medium text-court-ink/70 mb-2">Promo Code (optional)</p>
                {promoApplied ? (
                  <div className="flex items-center justify-between rounded-xl bg-green-50 border-2 border-green-300 px-4 py-3">
                    <div>
                      <p className="text-sm font-bold text-green-700">🎉 {promoApplied.message}</p>
                      <p className="text-xs text-green-600 mt-0.5">
                        Code <span className="font-mono font-bold">{promoCode.toUpperCase()}</span> applied — saving ₱{discountAmount}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={removePromo}
                      className="focus-ring ml-3 text-xs text-green-700 hover:underline font-semibold flex-shrink-0"
                    >
                      Remove
                    </button>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 rounded-xl border-2 border-court-ink/15 px-3 py-2 text-sm font-mono uppercase tracking-wider focus:outline-none focus:border-court-orange"
                      maxLength={30}
                    />
                    <button
                      type="button"
                      ...
                      className="focus-ring rounded-xl bg-court-orange text-white px-4 py-2 text-sm font-semibold hover:bg-court-orange-dark disabled:opacity-50 whitespace-nowrap"
                    >
                      {promoLoading ? "…" : "Apply"}
                    </button>
                  </div>
              ...
              <div className="border-t border-court-ink/10 pt-3 space-y-1 text-sm">
                ...
                {promoApplied && (
                  <div className="flex justify-between text-green-600 font-semibold">
                    <span>Promo ({promoApplied.discountPercent}% off)</span>
