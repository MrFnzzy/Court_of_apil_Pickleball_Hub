...
import RentalPickerCards from "@/components/RentalPickerCards";
import RecaptchaCheckbox from "@/components/RecaptchaCheckbox";
import SeasonalRateNoticeModal from "@/components/SeasonalRateNoticeModal";
import { rectOf, Rect } from "@/lib/promoFxTiming";
import { compressImageIfNeeded } from "@/lib/imageCompress";
import { uploadFile } from "@/lib/uploadFile";
import { labelForSlot, priceForSlot, overrideForDate, rentalPrice, ballPrice, DEFAULT_PRICING, PricingSettings, RentalProduct } from "@/lib/pricing";
import { ACTIVE_BOOKING_REF_KEY } from "@/lib/activeBookingRef";

// Response bodies aren't always JSON — a platform-level error (e.g. a
// request that's too large) can return a plain-text/HTML page instead,
// which would otherwise crash `res.json()` with a cryptic
// "Unexpected token" error. Parse safely and fall back to a readable message.
async function parseJsonSafe(res: Response): Promise<any> {
  ...
  try {
    ...
    if (res.status === 413 || /request entity too large/i.test(text)) {
      return { error: "That file is too large to upload. Please try a smaller screenshot." };
    }
    return { error: `Something went wrong (server returned an unexpected response). Please try again.` };
  }
}
...
// Purely visual: how each wizard step slides/fades in and out. `direction`
// (1 = advancing, -1 = going back) only ever changes the animation, never
// which step renders or how it's reached.
...
export default function BookPage() {
  ...
  // Tracks whether the wizard is moving forward or back so the step
  // transition (see stepVariants below) can slide the right direction —
  // purely visual, never affects which step is shown or how it's reached.
  ...
  // booking can combine slots from several different dates (e.g. 11PM-12AM
  // tonight + 12AM-1AM tomorrow + a slot on Sept 10, or any other mix of
  // dates) without losing earlier picks when the calendar moves to a
  // different day.
  ...
  // Synchronous re-entrancy lock. `promoChecking` (state) is only checked
  // by the *button's* disabled prop, and React state updates aren't
  // applied instantly — so an Enter-key auto-repeat (or a fast double
  // click before the re-render lands) could fire applyPromo() twice
  // before the button actually disables itself. A ref updates
  // synchronously, so it reliably blocks the second call.
  ...
  // Whichever seasonal pricing override (if any) covers the currently
  // selected date — drives the "rates changed" popup below. Null on the
  // vast majority of dates, which just use the main rates.
  const seasonalOverride = useMemo(
    () => overrideForDate(new Date(date + "T00:00:00.000Z"), pricing),
    [date, pricing]
  );
  ...
  // Only require a checked reCAPTCHA when a site key is actually configured
  // (see RecaptchaCheckbox — it renders nothing otherwise, same fail-open
  // behavior the site already had).
  ...
  async function applyPromo() {
    ...
    if (!raw) return;
    if (applyingPromoRef.current) return; // already mid-flight — ignore repeat triggers
    ...
    try {
      ...
      console.log("[promoFx trace] validate response:", data);
      if (!data.valid) {
        ...
        console.log("[promoFx trace] promoBoxRef.current:", promoBoxRef.current, "sourceRect:", sourceRect, "fxPlay currently:", fxPlay);
        ...
        if (!fxPlay && sourceRect) {
          console.log("[promoFx trace] setFxPlay firing — animation should mount now.");
          setFxPlay({ sourceRect, oldTotalText, newTotalText, percentage: data.percentage });
        } else {
          console.log("[promoFx trace] SKIPPED setFxPlay — fxPlay was truthy or sourceRect was null. This is why nothing plays.");
        }
      }
    } catch (err) {
      console.log("[promoFx trace] fetch/validate threw:", err);
  ...
  function validateClient(): string | null {
    ...
    if (recaptchaConfigured && !recaptchaToken) return "Please check the box to verify you're not a robot.";
    return null;
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      // 1. Shrink the screenshot client-side if it's large (keeps mobile
      //    uploads fast) and upload it straight to Blob storage.
      const compressed = await compressImageIfNeeded(file as File);
      const uploadData = await uploadFile(compressed, "proofs");
      ...
      setRedirecting(true);
      // Stash the reference code so PendingBookingLock (mounted in the root
      // layout) brings this browser straight back to the status page on
      // every visit — even after fully closing the site — until an admin
      // approves or rejects it.
      try {
        localStorage.setItem(ACTIVE_BOOKING_REF_KEY, data.bookingRef);
      } catch {
        // Private-browsing/storage-blocked: the customer can still reach
        // their status page manually via /track using the reference
        // number, they just won't get auto-redirected back to it.
      }
      router.push(`/track/${encodeURIComponent(data.bookingRef)}`);
    } catch (err: any) {
    ...
    return (
      <>
        ...
        <main className="max-w-2xl mx-auto px-4 sm:px-6 py-24">
          <div className="wizard-step rounded-court glass-panel p-8 sm:p-10 text-center">
            ...
            <p className="text-court-ink/70">Taking you to your booking status page…</p>
          </div>
        </main>
        <SiteFooter />
      </>
  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        <div className="wizard-step rounded-court glass-panel p-5 sm:p-6 mb-6">
        ...
        <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.section
              ...
              animate="center"
              exit="exit"
              className="rounded-court glass-panel p-5 sm:p-6"
            >
              ...
              {todayFullNotice && (
                ...
                <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto overscroll-contain bg-black/40 p-4" role="dialog" aria-modal="true">
                  <div className="relative w-full max-w-sm max-h-[85vh] overflow-y-auto rounded-court glass-panel glass-scroll p-6 text-center">
                    <button
                      type="button"
              ...
              <SeasonalRateNoticeModal override={seasonalOverride} />
            ...
            >
              ...
              <RentalPickerCards type="PADDLE" products={rentalProducts} selected={paddleCount} onSelect={setPaddleCount} />
              <button
                type="button"
              ...
              <div className="mt-8 pt-6 border-t border-court-ink/10">
                ...
                <RentalPickerCards type="BALL" products={rentalProducts} selected={ballCount} onSelect={setBallCount} />
                <button
                  type="button"
                ...
                {appliedPromo ? (
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyDown={(e) => { if (e.key === "Enter" && !promoChecking) { e.preventDefault(); applyPromo(); } }}
            ...
            >
              ...
              <RecaptchaCheckbox onVerify={setRecaptchaToken} onExpire={() => setRecaptchaToken(null)} />
            </motion.section>
            )}
            </AnimatePresence>
          ...
          <div className="lg:col-span-1">
            <div className="wizard-step sticky top-6 rounded-court glass-panel p-5 sm:p-6 overflow-hidden" style={{ animationDelay: "140ms" }}>
    ...
    <div className={`flex items-center ${last ? "" : "flex-1"}`}>
      ...
      >
        <motion.span
          ...
          className={`step-badge relative inline-flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold border-2 ${
            current
              ? "bg-court-orange border-court-orange text-white shadow-court"
              : done
              ? "bg-court-orange border-court-orange text-white shadow-court"
              : "bg-white border-court-ink/20 text-court-ink/60"
          }`}
        >
          {current && (
            <motion.span
              layoutId="step-dot-ring"
              className="absolute -inset-1 rounded-full ring-4 ring-court-orange/20"
        ...
        <span className={`hidden sm:block text-[11px] font-semibold whitespace-nowrap ${done || current ? "text-court-ink" : "text-court-ink/60"}`}>
          {label}
        </span>
      </button>
    </div>
