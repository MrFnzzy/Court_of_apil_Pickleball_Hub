 ...
 * meeting a ball) followed by a cheerful ascending 4-note chime, plus
 * an original spoken "Dink dink, new booking, don't blink!" cue
 * (browser text-to-speech — not a recording of anyone else's branded
 * notification sound). It's built to be noticeable but pleasant —
 * nothing here hits full gain or uses a harsh waveform — and it still
 * repeats so it can't be missed even if the admin has stepped away
 * from the desk. Call startRepeatingAlarm() to begin; it returns a
 * stop() function so the caller can cut it off early (e.g. the moment
 * the admin opens the notification bell, or the pending count drops
 * back to zero).
 *
 ...
 * poll — not a click — the very first cue after a fresh page load can
 ...
 * speechSynthesis ahead of time so later automatic cues aren't
 * silently swallowed.
 *
...
const RHYME_TEXT = "Dink dink, new booking, don't blink!";
...
/** A cheerful ascending 4-note chime (C5–E5–G5–C6, a bright major
 * arpeggio) using soft triangle-wave tones with a gentle attack and
 * release on each note — an upbeat "you've got a booking!" flourish,
 * not an alert klaxon. */
function playFourNoteChime(ctx: AudioContext, startTime: number) {
  ...
  notes.forEach((freq, i) => {
    ...
    gain.gain.exponentialRampToValueAtTime(0.0001, t + noteLen - 0.01);
...
/** The full cue: a paddle tap, then (once it settles) the 4-note chime.
 * Total length is a little over half a second, well before the spoken
 * cue starts. */
function playCheerfulCue(ctx: AudioContext, startTime: number) {
  playPaddleTap(ctx, startTime);
  playFourNoteChime(ctx, startTime + 0.16);
}

/** Original spoken cue using the browser's built-in text-to-speech —
 * a rhyming, upbeat "Dink dink, new booking, don't blink!" announcement.
 * This is browser TTS synthesizing our own text, not a recording of any
 * third party's branded notification sound. Silently no-ops if the
 * browser has no speechSynthesis support or no voices installed. */
 ...
 * of seconds apart — each a paddle tap + 4-note chime, with the spoken
 * "Dink dink, new booking, don't blink!" cue on every other round so the
 * voice doesn't get repetitive. Returns a stop() function to cancel any
 * rounds that haven't played yet. Automatically stops after `times`
 * plays. Does nothing (and returns a no-op stop) if muted or Web Audio
 * isn't available.
 */
export function startRepeatingAlarm(times = 100): { stop: () => void } {
  ...
  let stopped = false;
  const roundInterval = 2.2; // seconds between rounds — leaves room for the chime + voice cue
  ...
  function scheduleNext() {
    ...
    if (played % 2 === 0) announce(RHYME_TEXT);
    played += 1;
    if (played < times) {
...
/** Plays a single round (paddle tap + chime + voice cue) immediately,
 * ignoring the mute flag — used by the "Test alarm sound" button so the
 * admin can always check what it sounds like (and, as a side effect,
 * this click also unlocks audio for this tab per the autoplay-policy
 * note above). */
...
/**
 * Plays a single round (paddle tap + chime + voice cue), respecting the
 * mute flag — used when a real Web Push notification arrives (see
 * sw.js) while this tab is open. The Web Notifications API has no way
 * to attach a custom sound file to the OS-level notification itself
 * (every major browser ignores anything but its own default alert
 * tone there), so the service worker posts a message to any open
 * admin tab and this is how that tab plays our actual branded cue
 * instead of just the phone's generic ping. Does nothing if muted, if
 * Web Audio isn't available, or if audio hasn't been unlocked yet in
 * this tab (see unlockAudio) — same constraints as the polling-driven
 * alarm above.
 */
