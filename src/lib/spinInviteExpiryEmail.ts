...
import { getSpinWheelSettings, computeInviteExpiresAt } from "./spinWheelSettings";
...
/**
 * Applies the current inviteExpiryDays policy retroactively to invites
 * sent *before* the expiry feature existed (their expiresAt is still
 * null, so without this they'd stay spinnable forever and never get a
 * reminder). Each backfilled invite gets expiresAt = its own sentAt +
 * inviteExpiryDays — the same 2-week (or whatever the admin has set)
 * window every new invite gets, just measured from when it actually went
 * out rather than from today. An invite sent long enough ago will end up
 * with an expiresAt already in the past, which is correct: it's expired
 * under the policy exactly like a fresh invite would be after sitting
 * unused that long.
 *
 * Only touches invites still in play (unspun, un-revoked) — a spin
 * someone already used doesn't need an expiry date. If inviteExpiryDays
 * is currently null (admin has "never expires" selected), this is a
 * no-op: there is no policy to backfill.
 */
async function backfillMissingInviteExpiries(): Promise<void> {
  const settings = await getSpinWheelSettings();
  if (settings.inviteExpiryDays == null) return;

  const unbackfilled = await prisma.spinInvite.findMany({
    where: { expiresAt: null, spunAt: null, revokedAt: null },
    select: { id: true, sentAt: true },
  });

  for (const invite of unbackfilled) {
    const expiresAt = computeInviteExpiresAt(settings.inviteExpiryDays, invite.sentAt);
    // Guard the where clause the same way the reminder claim below does,
    // so two overlapping sweeps can't race on the same row.
    await prisma.spinInvite.updateMany({
      where: { id: invite.id, expiresAt: null },
      data: { expiresAt },
    });
  }
}
...
export async function sendSpinInviteExpiryReminders(): Promise<void> {
  await backfillMissingInviteExpiries();
