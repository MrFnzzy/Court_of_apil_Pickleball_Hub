...
export function playPromoFxSequence(): () => void {
  ...
  // Creating (or scheduling into) an AudioContext can throw — most notably
  // Safari's hard cap on simultaneous unclosed AudioContexts, which raises
  // immediately on `new AudioContext()` once the limit is hit (e.g. after
  // replaying this animation several times in a row during testing). This
  // function is called synchronously from PromoSuccessAnimation's effect
  // *before* the canvas render loop starts, so an uncaught throw here used
  // to abort that whole effect — killing the visual animation along with
  // the sound. Sound is a bonus; it must never be able to take the
  // visuals down with it.
  let ctx: AudioContext;
  try {
    ctx = new AC();
  } catch (err) {
    console.error("[promoFx] Could not create AudioContext, continuing without sound:", err);
    return () => {};
  }
  ...
  try {
    ...
    // Scheduling failed partway through — close what we opened and carry
    // on silently rather than letting this propagate into the caller.
    console.error("[promoFx] Sound scheduling failed, continuing without sound:", err);
    try {
      ctx.close();
    } catch {
      /* ignore */
