...
// Sent once, a few days before an unused spin invite itself expires (the
// link in /spin/[token] — separate from sendDiscountExpiryReminderEmail
// above, which reminds about a *won* code that's expiring). This is for a
// customer who never used their one spin at all. See
// spinInviteExpiryEmail.ts for exactly when this fires.
export async function sendSpinInviteExpiryReminderEmail(params: {
  email: string;
  customerName: string;
  token: string;
  expiresAt: Date;
  ...
  const firstName = params.customerName.trim().split(" ")[0] || "there";
  const spinUrl = `${siteBaseUrl()}/spin/${params.token}`;
  const expiresLabel = params.expiresAt.toLocaleDateString("en-PH", {
    timeZone: "Asia/Manila",
    month: "long",
    day: "numeric",
  ...
  const html = `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;">
        <div style="background:linear-gradient(135deg,#F46036,#6CD4FF);padding:24px;border-radius:16px 16px 0 0;">
          <h1 style="color:white;margin:0;font-size:22px;">Your spin is about to expire! 🎡</h1>
        </div>
        <div style="border:1px solid #eee;border-top:none;padding:24px;border-radius:0 0 16px 16px;">
          <p>Hi ${firstName},</p>
          <p>You still have an unused spin on Heide's Pickleball Hub's prize wheel — but it expires in <strong>${params.daysLeft} day${params.daysLeft === 1 ? "" : "s"}</strong>, on <strong>${expiresLabel}</strong>. After that, this link won't work anymore.</p>
          <div style="text-align:center;margin:28px 0;">
          ...
          <p>It only takes a few seconds, and you might win a discount for your next booking!</p>
          <p style="color:#999;font-size:12px;margin-top:24px;">Heide's Pickleball Hub — Talisay City, Cebu</p>
        </div>
      </div>
    `;

  await sendMail(params.email, `Your spin expires in ${params.daysLeft} day${params.daysLeft === 1 ? "" : "s"} — don't miss it!`, html);
}
