...
// Paddle rental packages. Balls are no longer bundled free with a paddle —
// see ballPackages()/ballPrice() below for their own separate pricing.
export function rentalPackages(
  settings: PricingSettings = DEFAULT_PRICING
): Record<number, { price: number; label: string }> {
  return {
    0: { price: 0, label: "No rental" },
    1: { price: settings.rental1Price, label: "1 paddle" },
    2: { price: settings.rental2Price, label: "2 paddles" },
  };
}
...
// Ball rental packages — a separate, paid add-on from the paddle rental.
export function ballPackages(
  settings: PricingSettings = DEFAULT_PRICING
): Record<number, { price: number; label: string }> {
  return {
    0: { price: 0, label: "No balls" },
    1: { price: settings.ball1Price, label: "1 ball" },
    3: { price: settings.ball3Price, label: "3 balls" },
  };
}

export function ballPrice(ballCount: number, settings: PricingSettings = DEFAULT_PRICING): number {
  return ballPackages(settings)[ballCount]?.price ?? 0;
}
