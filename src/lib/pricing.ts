 ...
 *
 * Mon-Fri day slots  -> weekdayDayPrice   (5:00 AM - 4:59 PM, hours 5..16)
 * Mon-Fri night slots -> weekdayNightPrice (5:00 PM - 4:59 AM, hours 17..23 & 0..4)
 * Sat & Sun -> weekendPrice (the court's actual bookable hours are 5:00 AM -
 * 1:00 AM, enforced separately via closedHours — this rate just applies to
 * whichever weekend hours are open, no day/night split like weekdays get)
 *
...
export function priceForSlot(date: Date, hour: number, settings: PricingSettings = DEFAULT_PRICING): number {
  ...
  // Weekday
  const isDaySlot = hour >= 5 && hour < 17; // 5AM - 4:59PM
