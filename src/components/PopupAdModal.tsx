...
export default function PopupAdModal({ ad }: { ad: PopupAdData | null }) {
  ...
  useEffect(() => {
    if (!visible || !ad?.videoUrl || videoFailed) return;
  ...
  }, [visible, ad?.videoUrl, videoFailed]);
      ...
      >
        ...
        >
          ...
          {/* Video plays edge-to-edge at its natural aspect ratio (no
              ...
              square, object-contain box when there's no video, OR when the
              browser can't decode the uploaded video (e.g. an iPhone .mov
              using HEVC, which most non-Safari browsers can't play — the
              poster frame would otherwise just sit there frozen forever). */}
          {ad.videoUrl && !videoFailed ? (
