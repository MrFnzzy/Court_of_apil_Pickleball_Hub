...
export default function DatePicker({
  ...
  /** Admin schedule/booking views only: lets the admin navigate to and
   * pick past dates (e.g. to log a walk-in after the fact, or just look
   * back at an earlier day's schedule) instead of being floored at
   * today like the customer-facing booking flow. Customers never get
   * this — they should never be able to "book" a day that's already
   * gone. */
  admin?: boolean;
}) {
  const todayStr = manilaToday();
  const min = admin ? "2000-01-01" : todayStr;
