 ...
 * It adds a wavy cover only around standard same-window link navigation, then
 * sends the browser to the original href after the cover is complete.
 */
...
const WAVE_STORAGE_KEY = "heidesPickleballHub.pageWave";
const WAVE_COVER_MS = 610;
...
function makeWave(x: number, y: number, phase: "cover" | "reveal") {
  const wave = document.createElement("div");
  wave.className = `rally-page-wave rally-page-wave--${phase}`;
  wave.setAttribute("aria-hidden", "true");
  wave.style.setProperty("--wave-x", `${x}px`);
  wave.style.setProperty("--wave-y", `${y}px`);
  ["rally-page-wave__ripple", "rally-page-wave__ripple", "rally-page-wave__foam", "rally-page-wave__ball"].forEach((className) => addLayerChild(wave, className));
  return wave;
}
...
export default function MotionDirector() {
  useEffect(() => {
    ...
    try {
      const savedWave = sessionStorage.getItem(WAVE_STORAGE_KEY);
      if (savedWave) {
        sessionStorage.removeItem(WAVE_STORAGE_KEY);
        const { x, y } = JSON.parse(savedWave) as { x: number; y: number };
        const arrivingWave = makeWave(x, y, "reveal");
        body.appendChild(arrivingWave);
        window.setTimeout(() => arrivingWave.remove(), 900);
      }
    } catch {
    ...
    const onNavigationClick = (event: MouseEvent) => {
      ...
      const wave = makeWave(event.clientX || window.innerWidth / 2, event.clientY || window.innerHeight / 2, "cover");
      body.appendChild(wave);
      try {
        sessionStorage.setItem(WAVE_STORAGE_KEY, JSON.stringify({ x: event.clientX || window.innerWidth / 2, y: event.clientY || window.innerHeight / 2 }));
      } catch {
        // No stored reveal is still a valid navigation.
      }
      window.setTimeout(() => window.location.assign(anchor.href), WAVE_COVER_MS);
    };
