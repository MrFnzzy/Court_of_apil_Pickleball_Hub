 ...
 *
 * This controller observes and decorates existing UI surfaces only. It does
 * not intercept clicks, alter form values, navigate, fetch data, or change
 * application state. Every added node is pointer-events:none and removed on
 * cleanup, keeping the supplied booking and reward flows intact.
 */
...
export default function MotionDirector() {
  useEffect(() => {
    ...
    const spawnServeImpact = (x: number, y: number) => {
      const impact = document.createElement("span");
      impact.className = "rally-serve-impact";
      impact.style.left = `${x}px`;
      impact.style.top = `${y}px`;
      ["rally-serve-impact__court", "rally-serve-impact__ball", "rally-serve-impact__spark", "rally-serve-impact__spark", "rally-serve-impact__spark"].forEach((className) => addLayerChild(impact, className));
      body.appendChild(impact);
      window.setTimeout(() => impact.remove(), 980);
    };
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      spawnServeImpact(event.clientX, event.clientY);
      target.classList.add("rally-pressed");
      window.setTimeout(() => target.classList.remove("rally-pressed"), 240);
    };
