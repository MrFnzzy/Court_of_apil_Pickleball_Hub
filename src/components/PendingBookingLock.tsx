...
export default function PendingBookingLock() {
  ...
  useEffect(() => {
    if (!pathname) return;
    // Never touch the admin dashboard, or any standalone page a customer
    // reaches via their own personal link — a spin-wheel invite or a
    // feedback request is meant to work regardless of whether they happen
    // to have an unrelated booking still pending elsewhere. Redirecting
    // them away from a link they were specifically sent is a bug, not a
    // safety feature; the lock is only about stopping a second *booking*
    // from being started while the first is unverified.
    if (
