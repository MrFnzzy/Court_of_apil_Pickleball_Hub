...
export default function PickleballShader() {
  ...
  useEffect(() => {
    ...
    let frame = 0; let pointerX = 0; let pointerY = 0; let lastFrame = 0;
    const resize = () => { const dpr = Math.min(window.devicePixelRatio || 1, window.innerWidth < 700 ? 0.85 : 1.15); canvas.width = window.innerWidth * dpr; canvas.height = window.innerHeight * dpr; gl.viewport(0, 0, canvas.width, canvas.height); };
    const move = (event: PointerEvent) => { pointerX = event.clientX / window.innerWidth * 2 - 1; pointerY = 1 - event.clientY / window.innerHeight * 2; };
    const render = (now: number) => { if (document.hidden) return; if (now - lastFrame < 33 && !reduceMotion) { frame = requestAnimationFrame(render); return; } lastFrame = now; gl.uniform1f(timeLocation, now * 0.001); gl.uniform2f(resolutionLocation, canvas.width, canvas.height); gl.uniform2f(pointerLocation, pointerX, pointerY); gl.drawArrays(gl.TRIANGLES, 0, 6); if (!reduceMotion) frame = requestAnimationFrame(render); };
    const onVisibility = () => { if (!document.hidden && !reduceMotion) { lastFrame = 0; cancelAnimationFrame(frame); frame = requestAnimationFrame(render); } }; resize(); document.addEventListener("visibilitychange", onVisibility); window.addEventListener("resize", resize); window.addEventListener("pointermove", move, { passive: true }); render(0);
    return () => { cancelAnimationFrame(frame); document.removeEventListener("visibilitychange", onVisibility); window.removeEventListener("resize", resize); window.removeEventListener("pointermove", move); };
  }, []);
