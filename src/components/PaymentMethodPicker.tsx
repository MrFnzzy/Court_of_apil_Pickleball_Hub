        ...
        {(["GCASH", "MAYA", "BPI"] as const).map((m) => {
          ...
          return (
            <button
              ...
              disabled={!has}
              onClick={() => onChange(m)}
              title={!has ? `${meta.label} isn't available right now` : undefined}
              className={`focus-ring rounded-xl border-2 px-3 py-3 text-center font-semibold text-sm transition-all disabled:cursor-not-allowed ${
                value === m
                  ? "border-court-orange bg-court-orange/10 shadow-court text-court-ink"
                  : has
                  ? "border-court-ink/10 bg-white hover:border-court-blue-dark/40 text-court-ink"
                  : "border-court-ink/10 bg-court-ink/5 text-court-ink/60"
              }`}
            >
              <span
                className={`mx-auto mb-1 block h-2.5 w-2.5 rounded-full ${meta.color} ${has ? "" : "opacity-50"}`}
              />
              {meta.label}
              {!has && <span className="block text-[9px] font-normal normal-case text-court-ink/50">Unavailable</span>}
            </button>
          );
        })}
      </div>
