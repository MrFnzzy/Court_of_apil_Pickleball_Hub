...
// "Sep 1", "Sep 1 and Sep 10", or "Sep 1, Sep 10, and Sep 20" — reads
// naturally for any number of combined dates, not just a pair.
...
export type EditableBooking = {
   ...
   * fetched by the caller when the booking has a groupId — lets a
   * multi-day booking be edited as a whole instead of losing its other
   * dates. Falls back to a single-date booking when absent. */
  groupBookings?: { id: string; date: string; startHours: number[] }[];
};
...
export default function AdminManualBookingForm({
  ...
  // A single cart of { date, hour } picks, used by both modes. Create mode
  // starts empty; edit mode seeds it from the booking being edited — every
  // leg of it, when it's a linked multi-day booking (see `groupBookings`),
  // not just whichever single date's row was clicked — so every date stays
  // visible and editable together instead of the other dates silently
  // getting dropped on save. Kept across date navigation in both modes
  // (mirrors the customer-facing booking flow) so combining slots from
  // another date doesn't erase the earlier picks.
  const [cart, setCart] = useState<{ date: string; hour: number }[]>(() => {
    if (!editBooking) return [];
    const legs = editBooking.groupBookings && editBooking.groupBookings.length > 0
      ? editBooking.groupBookings
      : [{ id: editBooking.id, date: editBooking.date, startHours: editBooking.startHours }];
    return legs.flatMap((leg) => leg.startHours.map((hour) => ({ date: leg.date.slice(0, 10), hour })));
  });
  // Every booking id this edit covers (both legs of a linked booking, or
  // just the one) — so the schedule grid reports this booking's own slots
  // as available on EITHER date, not just the one currently on screen.
  ...
  const [isDownpayment, setIsDownpayment] = useState(editBooking?.isDownpayment ?? false);
  const [downpaymentNote, setDownpaymentNote] = useState(editBooking?.downpaymentNote ?? "");
  ...
  // currently displayed.
  const selectedHours = cart.filter((c) => c.date === bookingDate).map((c) => c.hour);

  // Cart grouped by date, sorted chronologically — every group gets
  // submitted together as one linked booking (create), or as the full
  // reshaped set of legs for an existing one (edit).
  ...
  }, [cart, paddleCount, ballCount, pricing, rentalProducts]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (groupsByDate.length > 20) {
      setError("A single booking can only combine slots from up to 20 dates.");
      return;
    }
      ...
      const res = isEditing
        ? await fetch(`/api/admin/bookings/${editBooking!.id}`, {
            ...
            body: JSON.stringify({
              ...
              isPaid,
              isDownpayment,
              downpaymentNote: isDownpayment ? downpaymentNote.trim() || undefined : undefined,
            }),
          })
  ...
  return (
    ...
    <div className="fixed inset-0 z-[100]" role="dialog" aria-modal="true">
      ...
      <div className="fixed top-3 sm:top-6 md:top-10 inset-x-3 sm:inset-x-0 sm:mx-auto w-auto sm:w-full sm:max-w-4xl max-h-[calc(100vh-1.5rem)] sm:max-h-[calc(100vh-3rem)] md:max-h-[calc(100vh-5rem)] overflow-y-auto overscroll-contain rounded-court glass-panel glass-scroll">
        ...
        {done ? (
          <div className="p-10 text-center">
            ...
            <p className="text-sm text-court-ink/60 mb-6">
              ...
              {isEditing || groupsByDate.length <= 1
                ? bookingDate
                : joinDates(groupsByDate.map((g) => g.date))}
              .
            </p>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6 p-5 sm:p-6">
            ...
            <div>
              ...
              <div key={gridKey}>
                <ScheduleGrid
                  ...
                  autoRefresh={false}
                  admin
                  excludeBookingId={excludeBookingIds}
                />
              </div>
            ...
            <div className="space-y-4">
              ...
              {/* Downpayment tag — independent of status/isPaid, purely
                  informational so it's visible in the booking list without
                  opening this form again. */}
              <label
                className={`flex items-start gap-2.5 rounded-xl border-2 px-3 py-2.5 text-sm cursor-pointer transition-colors ${
                  isDownpayment ? "border-court-orange/40 bg-court-orange/5" : "border-court-ink/10 hover:border-court-ink/20"
                }`}
              >
                <input
                  type="checkbox"
                ...
                />
                <span>
                  <span className="block font-medium text-court-ink/80">Downpayment only</span>
                  <span className="block text-xs text-court-ink/50">
                    Tag this booking as partially paid — shows a badge in the booking list so any admin can see it without opening it.
                  </span>
                </span>
              </label>
              {isDownpayment && (
                <label className="text-sm block">
                  <span className="block mb-1 font-medium text-court-ink/80">Downpayment note</span>
                  <input
                    value={downpaymentNote}
                    onChange={(e) => setDownpaymentNote(e.target.value)}
                    placeholder="e.g. ₱500 down, balance on arrival"
                    className="w-full rounded-xl border-2 border-court-ink/15 px-3 py-2 focus-ring"
                  />
                </label>
              )}
