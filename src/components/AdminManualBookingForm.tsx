  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/admin/bookings", {
        ...
        body: JSON.stringify({ customerName, contactNumber, email, date, hours, paddleCount, ballCount, status }),
      });
    ...
    <form onSubmit={handleSubmit} className="rounded-court bg-white border-2 border-court-orange/30 shadow-court p-5 sm:p-6 mb-6">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mt-5">
        ...
        <label className="text-sm">
          ...
          <select value={ballCount} onChange={(e) => setBallCount(Number(e.target.value) as 0 | 1 | 3)} className="w-full rounded-xl border-2 border-court-ink/15 px-3 py-2">
            <option value={0}>No rental</option>
            <option value={1}>1 ball (₱50)</option>
            <option value={3}>3 balls (₱120)</option>
          </select>
        </label>
        <label className="text-sm">
