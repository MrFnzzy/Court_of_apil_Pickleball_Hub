...
export type EditableBooking = {
   ...
   * fetched by the caller when the booking has a groupId — lets a 2-day
   * booking be edited as a whole instead of losing its other date. Falls
   * back to a single-date booking when absent. */
...
export default function AdminManualBookingForm({
  ...
  // leg of it, when it's a linked 2-day booking (see `groupBookings`), not
  // just whichever single date's row was clicked — so both dates stay
  // visible and editable together instead of the second date silently
  // getting dropped on save. Kept across date navigation in both modes
  // (mirrors the customer-facing booking flow) so combining slots from a
  // second date doesn't erase the first date's picks.
  ...
  // Cart grouped by date, sorted chronologically — up to two groups get
