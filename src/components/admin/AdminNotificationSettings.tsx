...
export default function AdminNotificationSettings() {
  ...
  return (
    <div className="rounded-court glass-panel p-4">
      <h3 className="font-display font-600 text-base text-court-ink mb-0.5">Push notifications</h3>
      <p className="text-xs text-court-ink/60 mb-3">
        Get a real notification on this device the moment a new booking comes in.
      </p>
      ...
      {(status === "subscribed" || status === "unsubscribed" || status === "default") && (
        <div className="flex items-center gap-2.5 flex-wrap">
          <button
            ...
            disabled={busy}
            className={`focus-ring rounded-full px-4 py-1.5 text-sm font-semibold shadow-court disabled:opacity-50 ${
              status === "subscribed"
          ...
          {status === "subscribed" && (
            <>
              ...
              <button
                ...
                onClick={sendTest}
                disabled={testBusy}
                className="focus-ring rounded-full px-3 py-1.5 text-sm font-semibold text-court-ink/70 border-2 border-court-ink/15 hover:bg-court-ink/5 disabled:opacity-50"
              >
