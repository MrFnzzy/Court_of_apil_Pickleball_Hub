...
export default function AdminRecentBookings() {
  ...
  // Today's bookings are pinned above the tabs and always shown, no matter
  // which range tab is selected — switching to "Past" shouldn't hide what's
  // happening today.
  const todayItems = useMemo(
    () =>
      bookings
        .filter((b) => b.date.slice(0, 10) === today)
        .sort((a, b) => Math.min(...a.startHours) - Math.min(...b.startHours)),
    [bookings, today]
  );
  ...
  // were created in. Today is excluded here since it's already pinned above.
          ...
          {todayItems.length > 0 && (
            <div>
              <div className="flex items-baseline justify-between gap-2 mb-2">
                <h4 className="font-display font-700 text-sm sm:text-base text-court-orange">
                  {groupHeaderLabel(today, today)}
                </h4>
