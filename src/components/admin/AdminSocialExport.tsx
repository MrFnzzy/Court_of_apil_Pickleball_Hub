...
// Compact axis label for a table row, e.g. "12A", "1A" ... "11P" — a full
// "12:00 – 1:00 AM" range would be too wide for a 24-row table squeezed
// next to two day columns.
...
const BUCKET_COLOR: Record<ReturnType<typeof bucketFor>, string> = {
  available: "#22c55e",
  booked: "#ef4444",
  unavailable: "#d1d5db",
};
...
function RowCells({ hour, schedule }: { hour: number; schedule: DaySchedule[] }) {
  const isRowEnd = hour === 23;
  return (
    <>
      <div
        className={`flex items-center justify-end pr-1 bg-court-ink/5 text-[8.5px] font-semibold text-court-ink/50 ${
          isRowEnd ? "" : "border-b border-court-ink/10"
        }`}
      >
      ...
      {schedule.map((day) => {
        ...
        return (
          <div
            ...
            className={`h-[15px] ${isRowEnd ? "" : "border-b border-white/40"}`}
          />
        );
      })}
    </>
...
function LegendDot({ color, label }: { color: string; label: string }) {
  return (
    <span className="inline-flex items-center gap-1 text-[9.5px] font-semibold text-court-ink/60">
...
function CanvasElementView({
  ...
  return (
    ...
    >
      ...
      {el.kind === "schedule" && (
        <div style={{ width: el.width }} className="rounded-xl bg-white/95 shadow-court overflow-hidden">
          {scheduleLoading && schedule.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "34px 1fr 1fr" }}>
              {/* header row */}
              <div className="bg-court-ink/5" />
              {schedule.map((day, i) => (
                <div key={day.date} className="bg-court-ink text-white text-center py-1.5 px-1">
          ...
          <div className="flex items-center gap-3 px-3 py-2 border-t border-court-ink/10 bg-white">
            <LegendDot color={BUCKET_COLOR.available} label="Available" />
            <LegendDot color={BUCKET_COLOR.booked} label="Booked" />
            <LegendDot color={BUCKET_COLOR.unavailable} label="Unavailable" />
          </div>
        </div>
      )}
