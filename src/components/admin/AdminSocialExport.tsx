...
// Full clock-style axis label for a table row, e.g. "12:00A", "1:00A" ...
// "11:00P" — deliberately compact (no space before the period) so it still
// fits a narrow time column, while reading as an actual clock time rather
// than a bare "1A" shorthand.
...
function RowCells({ hour, schedule }: { hour: number; schedule: DaySchedule[] }) {
  return (
    <>
      <div className="flex items-center justify-end pr-1 text-[7.5px] font-bold text-court-ink/45 tracking-tight leading-none">
      ...
      {schedule.map((day) => {
        ...
        return (
          <div
            ...
            className="relative h-[12px] rounded-full overflow-hidden"
          >
            {/* glossy "liquid" highlight near the top of the bubble */}
            <div
              className="absolute inset-x-1 top-[1px] h-[4px] rounded-full pointer-events-none"
      ...
      {el.kind === "schedule" && (
        ...
        >
          {scheduleLoading && schedule.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "38px 1fr 1fr", gap: 1.5, padding: 5 }}>
              {/* header row */}
              <div />
              {schedule.map((day, i) => (
                <div
                  ...
                  className="text-white text-center py-1 px-1 rounded-lg"
                >
                  <p className="font-display font-800 text-[11px] leading-none tracking-wide">
                    {i === 0 ? "TODAY" : "TOMORROW"}
                  </p>
                  <p className="text-[8px] leading-none mt-0.5 opacity-70">{dateSubLabel(day.date)}</p>
                </div>
              ))}
          ...
          <div className="flex items-center gap-3 px-3 py-1.5 border-t border-court-ink/10">
