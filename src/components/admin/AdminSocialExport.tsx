...
// Soft brand-colored glow that sits behind the card, built from a plain
// radial-gradient rather than layering on `filter: blur` — still reads as
// a glow even against a fully transparent export, so the card looks
// intentionally "lit" rather than dropped onto nothing.
function CardGlow() {
  return (
...
// Plain CSS (gradients/box-shadow) rather than backdrop-filter — the
// "glass" look comes from layered gradients instead.
//
...
export default function AdminSocialExport() {
  ...
  async function handleDownload() {
    ...
    // Let the "exporting" state re-render before we snapshot — this swaps
    // the editor-only transparency checkerboard (see the canvas style
    // below) out for a real transparent fill, and clears selection
    // outlines, so neither ends up baked into the PNG.
    await new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r)));
    try {
      // html-to-image renders through a real browser SVG <foreignObject>
      // instead of html2canvas's from-scratch reimplementation of CSS
      // painting. We switched to it because html2canvas was visibly
      // mis-rendering this design: it baked the editor's own frame border
      // into exports, and — because the schedule card sits inside a
      // `transform: scale(...)` wrapper, which html2canvas has long-known
      // stacking/compositing bugs with — it was painting the transparency
      // checkerboard on *top* of opaque elements instead of behind them.
      // Letting the browser do the actual painting sidesteps both.
      const { toPng } = await import("html-to-image");
      const dataUrl = await toPng(canvasRef.current, {
        pixelRatio: 2,
        backgroundColor: transparentBg && !bgImage ? undefined : background,
        cacheBust: true,
      });
          ...
          <div
            ...
            style={{
              ...
              background: bgImage
                ...
                : transparentBg
                ? exporting
                  // The checkerboard is only a design-time hint that the
                  // canvas is transparent. It's a real painted CSS
                  // background, so any capture library would happily
                  // include it verbatim — it has no way to know the
                  // pattern is meant to be a UI affordance rather than
                  // content. We swap it for an explicit "transparent"
                  // right before snapshotting (see handleDownload) so it
                  // can never end up baked into the PNG.
                  ? "transparent"
