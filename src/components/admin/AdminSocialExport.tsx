    ...
    >
      ...
      {el.kind === "schedule" && (
        <div
          style={{
            width: el.width,
            background: "linear-gradient(165deg, rgba(255,255,255,0.97) 0%, rgba(255,255,255,0.86) 100%)",
            border: "1.5px solid rgba(255,255,255,0.75)",
            boxShadow: "0 22px 45px rgba(15,23,42,0.35), 0 2px 8px rgba(15,23,42,0.12), inset 0 1px 0 rgba(255,255,255,0.9)",
            backdropFilter: "blur(6px)",
            borderRadius: 22,
          }}
          className="relative overflow-hidden"
        >
          {/* diagonal "liquid glass" sheen streak across the whole card */}
          <div
            className="absolute inset-0 pointer-events-none"
            style={{ background: "linear-gradient(115deg, transparent 25%, rgba(255,255,255,0.5) 42%, transparent 58%)" }}
          />
          {/* dink-dot texture (matches the site's own pickleball-ball motif), very faint */}
          <div
            className="absolute inset-0 pointer-events-none opacity-[0.35]"
            style={{
              backgroundImage: "radial-gradient(circle, rgba(31,70,83,0.35) 1px, transparent 1px)",
              backgroundSize: "9px 9px",
            }}
          />
          {/* court-line dashed border, inset from the card edge */}
          <div
            className="absolute pointer-events-none"
            style={{ inset: 5, borderRadius: 16, border: "1.5px dashed rgba(244,96,54,0.4)" }}
          />
          {/* ball watermark, tucked in the corner */}
          <BallIcon className="absolute -bottom-3 -right-3 h-16 w-16 text-court-orange/10 pointer-events-none" />
          ...
          <div className="relative">
            {scheduleLoading && schedule.length === 0 ? (
              ...
              <div className="relative" style={{ display: "grid", gridTemplateColumns: "50px 1fr 1fr", gap: 2, padding: 7 }}>
                ...
                <div
                  ...
                  style={{
                    left: "calc(50% + 26px)",
                    borderLeft: "2px dashed rgba(31,70,83,0.28)",
                  }}
                />

                {/* header row */}
                <div className="flex items-center justify-center">
                  <PaddleIcon className="h-5 w-5 text-court-orange-dark/70" />
                </div>
                {schedule.map((day, i) => (
                  <div
                    key={day.date}
                    style={{
                      background: "linear-gradient(165deg, #245666 0%, #0f2c36 100%)",
                      boxShadow: "0 3px 8px rgba(15,44,54,0.45), inset 0 1px 1px rgba(255,255,255,0.2)",
                    }}
                    className="relative overflow-hidden text-white text-center py-1.5 px-1 rounded-lg"
                  >
                    <div
                    ...
                    <p className="relative text-[9px] leading-none mt-0.5 opacity-75">{dateSubLabel(day.date)}</p>
                  </div>
                ))}
            ...
            <div className="relative flex flex-wrap items-center gap-1.5 px-3 py-2.5 border-t border-court-ink/10">
              ...
              <PaddleIcon className="h-3.5 w-3.5 text-court-ink/25 ml-auto" />
            </div>
          </div>
        </div>
      )}
