...
export default function AdminFeedback() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/feedback", { cache: "no-store" })
      ...
      .then((d) => {
        if (!cancelled) setFeedback(Array.isArray(d?.feedback) ? d.feedback : []);
      })
      .catch((err) => {
        if (!cancelled) {
          console.error(err);
          setError("Couldn't load feedback. Please refresh and try again.");
        }
      })
      .finally(() => {
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <p className="text-red-600 text-center py-12 text-sm">{error}</p>
      ) : feedback.length === 0 ? (
