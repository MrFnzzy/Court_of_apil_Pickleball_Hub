...
export default function AdminMusicSettings() {
  ...
  async function load() {
    ...
    try {
      ...
      const settings: MusicSettings = {
        musicUrl: data.settings?.musicUrl ?? null,
      ...
      console.error(err);
      setLoadError("Couldn't load music settings. Please refresh and try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    setUploading(true);
    try {
      const uploadData = await uploadFile(file, "music");
  ...
  if (loadError || !values) {
    return <p className="text-red-600 text-sm">{loadError || "Couldn't load music settings."}</p>;
  }
  ...
  return (
    <div className="space-y-6 max-w-xl">
      ...
      <div className="rounded-court glass-panel p-5 sm:p-6 flex items-start justify-between gap-4">
        <div>
