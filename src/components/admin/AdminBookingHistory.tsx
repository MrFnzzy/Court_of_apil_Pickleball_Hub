...
export default function AdminBookingHistory() {
  ...
  return (
    <div>
                ...
                <div className="space-y-3">
                  {group.items.map((b, i) => {
                    ...
                    return (
                      <div
                        key={b.id}
                        className={`wizard-step rounded-court bg-white border-2 shadow-court p-4 sm:p-5 transition-colors ${
                          isSelected ? "border-court-orange/50 bg-court-orange/5" : "border-court-ink/10"
                        }`}
      ...
      {deletingBooking && (
        // The overlay itself scrolls (rather than centering with a fixed
        // height) so the dialog is always reachable — on a short mobile
        // viewport, or with the on-screen keyboard open, content taller
        // than the screen no longer gets clipped with no way to reach it.
        <div className="fixed inset-0 z-[110] overflow-y-auto overscroll-contain" role="dialog" aria-modal="true">
          <button
            type="button"
