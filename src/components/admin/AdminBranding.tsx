...
export default function AdminBranding() {
  ...
  function handleWordmarkPick(file: File | null) {
    setWordmarkFile(file);
    setSaved(false);
    if (file) {
      setWordmarkPreview(URL.createObjectURL(file));
    } else {
  ...
  async function uploadImage(file: File): Promise<string> {
    const uploadData = await uploadFile(file, "branding");
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      let wordmarkLogoUrl = values.wordmarkLogoUrl;
      if (wordmarkFile) {
        wordmarkLogoUrl = await uploadImage(wordmarkFile);
      }

      const res = await fetch("/api/admin/site-settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...values, logoUrl, wordmarkLogoUrl }),
      });
  ...
  const displayedWordmark = wordmarkPreview || values.wordmarkLogoUrl;

  return (
    <form onSubmit={handleSave} className="rounded-court glass-panel p-5 sm:p-6 max-w-xl">
      ...
      <div className="flex items-center gap-4 mb-5">
        ...
        <label className="text-sm">
          ...
          />
          <span className="block mt-1 text-xs text-court-ink/50">Square image works best — it's shown as a small circular icon next to the site name. Leave empty to keep the paddle icon.</span>
        </label>
      </div>

      <div className="flex items-center gap-4 mb-5 pt-5 border-t border-court-ink/10">
        <div className="relative h-12 w-32 shrink-0 rounded-lg overflow-hidden border-2 border-court-ink/10 bg-court-cream flex items-center justify-center">
          {displayedWordmark ? (
            <Image src={displayedWordmark} alt="Wordmark logo preview" fill className="object-contain" />
          ) : (
            <span className="text-xs text-court-ink/40">No image</span>
          )}
        </div>
        <label className="text-sm">
          <span className="block mb-1 font-medium text-court-ink/80">Header logo image</span>
          <input
            type="file"
            accept="image/*"
            onChange={(e) => handleWordmarkPick(e.target.files?.[0] || null)}
            className="block text-sm"
          />
          <span className="block mt-1 text-xs text-court-ink/50">
            The full site name + tagline wordmark shown beside the round icon at the top of every page. Use a wide image with a transparent background. Leave empty to fall back to plain text.
          </span>
        </label>
      </div>
