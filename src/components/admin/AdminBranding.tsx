  ...
  return (
    <form onSubmit={handleSave} className="rounded-court bg-white border-2 border-court-orange/30 shadow-court p-5 sm:p-6 max-w-xl">
      ...
      <div className="flex items-center gap-4 mb-5">
        <div className="relative h-16 w-40 shrink-0 rounded-xl overflow-hidden border-2 border-court-ink/10 bg-court-cream flex items-center justify-center">
          {displayedLogo ? (
            <Image src={displayedLogo} alt="Logo preview" fill className="object-contain" />
          ) : (
        ...
        <label className="text-sm">
          ...
          />
          <span className="block mt-1 text-xs text-court-ink/50">Wide wordmark logos work best — the header shows it at full width, not cropped into a circle. Leave empty to keep the default logo.</span>
        </label>
      </div>
