...
export default function AdminSecuritySettings() {
  ...
  async function revokeDevice(id: string) {
    ...
    try {
      ...
      console.error(err);
      alert("Couldn't revoke that device. Please try again.");
    } finally {
  ...
  async function revokePasskey(id: string) {
    ...
    try {
      const res = await fetch("/api/admin/security/passkeys", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id }),
      });
      ...
      console.error(err);
      alert("Couldn't revoke that passkey. Please try again.");
    } finally {
