...
export default function AdminHeroSlides() {
  ...
  async function load() {
    ...
    try {
      ...
      const slidesArr = Array.isArray(data?.slides) ? data.slides : [];
      setSlides(slidesArr.sort((a: Slide, b: Slide) => a.order - b.order));
      setLoadError(null);
    } catch (err) {
      console.error(err);
      setLoadError("Couldn't load the slideshow. Please refresh and try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setUploading(true);
    try {
      const uploadData = await uploadFile(file, "hero");
  ...
  async function toggleActive(slide: Slide) {
    try {
      ...
      console.error(err);
      alert("Couldn't update that slide. Please try again.");
    }
  }
  ...
  async function move(slide: Slide, direction: "up" | "down") {
    ...
    try {
      ...
      if (results.some((r) => !r.ok)) throw new Error("Failed to reorder slides.");
      await load();
    } catch (err) {
      console.error(err);
      alert("Couldn't reorder the slides. Please try again.");
    }
  }

  return (
    <div className="space-y-6">
      <form onSubmit={handleAdd} className="rounded-court glass-panel p-5 sm:p-6">
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {slides.map((slide, i) => (
              <div key={slide.id} className="rounded-court glass-panel p-4 flex gap-4 items-center">
