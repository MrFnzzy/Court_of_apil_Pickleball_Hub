...
export default function AdminPaymentAccounts() {
  ...
  async function load() {
    ...
    try {
      ...
      console.error(err);
      setLoadError("Couldn't load payment accounts. Please refresh and try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      if (qrFile) {
        const uploadData = await uploadFile(qrFile, "proofs");
  ...
  async function remove(id: string) {
    ...
    try {
      ...
      console.error(err);
      alert("Couldn't remove that account. Please try again.");
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {accounts.map((acc) => (
              <div key={acc.id} className="rounded-court glass-panel p-4 flex gap-3">
                {acc.qrImageUrl && (
