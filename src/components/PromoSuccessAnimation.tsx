...
export default function PromoSuccessAnimation({
  ...
  //
  // ROOT CAUSE FIX: the target <span> has key={grandTotal}, so React
  // unmounts + remounts it when the promo is applied (same React batch as
  // this component mounting). A single rAF can still fire before the browser
  // has completed layout for the new element. Two nested rAFs guarantee we
  // measure AFTER the first fully-painted frame of the new DOM node.
  useEffect(() => {
    ...
    // Capture computed text style synchronously — safe before layout settles.
    ...
    // Measure and update the fixed-position target overlay.
    // getBoundingClientRect() returns viewport-relative coords which is
    // exactly what position:fixed needs — no scroll offset adjustment needed.
    function measureAndUpdate() {
      const current = targetRef.current;
      if (!current) return;
      setTargetRect((prev) => {
        const r = current.getBoundingClientRect();
        // Skip re-render if position hasn't meaningfully changed (< 1px).
        if (
          prev &&
    ...
    // Throttled listener: schedule a measurement in the next rAF to avoid
    // layout thrashing from many rapid scroll/resize events.
    ...
    // Double-rAF initial measurement: ensures new keyed element has settled.
    raf1 = requestAnimationFrame(() => {
      raf2 = requestAnimationFrame(() => {
        measureAndUpdate();

        // Live tracking for the rest of the animation's lifetime.
        window.addEventListener("scroll", onViewportChange, { passive: true, capture: true });
        window.addEventListener("resize", onViewportChange, { passive: true });

        // ResizeObserver picks up layout shifts from dynamic content (e.g.
        // the promo chip expanding the box) that scroll/resize miss.
        const ro = new ResizeObserver(onViewportChange);
        ro.observe(document.documentElement);
        if (targetRef.current) ro.observe(targetRef.current);

        // Cleanup lives in a nested effect return so React manages it.
    ...
    <ModalPortal lockScroll={false}>
      <div className="promofx-layer" aria-hidden="true" role="presentation">
        ...
        {/* Price stage: masks the real total and plays out phases 3-7.
            The target div is always positioned from a live rect so it never
            drifts even if the user scrolls mid-animation. */}
