...
export interface PromoSuccessAnimationProps {
   ...
   * re-render (see book/page.tsx) — where the pickleball spawns from. */
  sourceRect: Rect;
...
// ---------- sprite assets (ball + paddle) ----------
// Loaded once at module scope so they're already decoded by the time the
// first animation actually runs (kicked off by a user action, so there's
// always at least a little lead time). `typeof Image` guards SSR/build.
const ballSprite: HTMLImageElement | null = typeof Image !== "undefined" ? new Image() : null;
if (ballSprite) ballSprite.src = "/promo-fx/ball.png";

const paddleSprite: HTMLImageElement | null = typeof Image !== "undefined" ? new Image() : null;
if (paddleSprite) paddleSprite.src = "/promo-fx/paddle.png";

function spriteReady(img: HTMLImageElement | null): img is HTMLImageElement {
  return !!img && img.complete && img.naturalWidth > 0;
}

// Source paddle.png is 293x543 with the face-to-handle "neck" (the point
// the whole paddle rotates/scales around) at (144, 359) in image pixels.
const PADDLE_IMG_W = 293;
const PADDLE_IMG_H = 543;
const PADDLE_NECK_X = 144;
const PADDLE_NECK_Y = 359;
// Matches the old vector paddle's face height (faceTop -40 to faceBottom
// 4), so the sprite reads at the same size/speed the timing was tuned for.
const PADDLE_FACE_LOCAL_H = 44;
const PADDLE_IMG_SCALE = PADDLE_FACE_LOCAL_H / PADDLE_NECK_Y;
...
/** Court-line pulses make the completed voucher event read as a scored rally.
 * This canvas layer is decorative and never touches promo validation. */
function drawCourtPulse(ctx: CanvasRenderingContext2D, x: number, y: number, progress: number, cyan: string, orange: string) {
  const p = clamp01(progress);
  const size = lerp(26, 150, easeOutCubic(p));
  const alpha = (1 - p) * 0.5;
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(-0.22);
  ...
  ctx.strokeRect(-size, -size * 0.54, size * 2, size * 1.08);
  ctx.beginPath();
...
function drawRallyTrajectory(ctx: CanvasRenderingContext2D, x1: number, y1: number, x2: number, y2: number, progress: number, cyan: string, orange: string) {
  const p = clamp01(progress);
  const controlX = lerp(x1, x2, 0.48);
  const controlY = Math.min(y1, y2) - Math.max(54, Math.abs(x2 - x1) * 0.18);
  ctx.save();
  ctx.lineWidth = 2.2;
  ctx.setLineDash([5, 9]);
  ctx.lineDashOffset = -p * 68;
  ctx.strokeStyle = cyan;
  ...
  ctx.beginPath();
  ctx.moveTo(x1, y1 + 3);
  ctx.quadraticCurveTo(controlX, controlY + 4, x2, y2 + 3);
  ctx.stroke();
  ctx.restore();
}
...
// Fixed scatter of pickleball-hole positions, as unit-circle offsets
// (multiplied by radius at draw time). Roughly even coverage of the
// visible face without looking like a perfect grid.
const PICKLEBALL_HOLES: [number, number][] = [
  [0, -0.62], [0.32, -0.5], [-0.34, -0.48], [0.58, -0.22], [-0.56, -0.24],
  [0.14, -0.16], [-0.18, -0.02], [0.5, 0.08], [-0.5, 0.12], [0.06, 0.32],
  [0.34, 0.42], [-0.3, 0.4], [0, 0.6],
];

function drawPickleball(ctx: CanvasRenderingContext2D, x: number, y: number, r: number, cyan: string) {
  if (spriteReady(ballSprite)) {
    ctx.save();
    ctx.shadowBlur = 18;
    ctx.shadowColor = cyan;
    ctx.drawImage(ballSprite, x - r, y - r, r * 2, r * 2);
    ctx.restore();
    return;
  }
  ...
  ctx.shadowBlur = 18;
  ctx.shadowColor = cyan;
  const grd = ctx.createRadialGradient(x - r * 0.28, y - r * 0.32, 0, x, y, r);
  grd.addColorStop(0, "#ffffff");
  grd.addColorStop(0.22, "#f5ff8a");
  grd.addColorStop(0.6, "#d8e620");
  grd.addColorStop(1, "#8a9c12");
  ctx.fillStyle = grd;
  ctx.beginPath();
  ctx.arc(x, y, r, 0, Math.PI * 2);
  ctx.fill();

  // Perforation holes — the pickleball's signature texture — clipped to
  // the ball's silhouette so they stay inside the circle at any radius.
  ctx.save();
  ctx.beginPath();
  ctx.arc(x, y, r, 0, Math.PI * 2);
  ctx.clip();
  ctx.shadowBlur = 0;
  ctx.fillStyle = "rgba(90, 100, 10, 0.55)";
  for (const [ox, oy] of PICKLEBALL_HOLES) {
    ctx.beginPath();
    ctx.arc(x + ox * r, y + oy * r, Math.max(1, r * 0.1), 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
  ctx.restore();
...
function drawPaddle(ctx: CanvasRenderingContext2D, x: number, y: number, rotDeg: number, scale: number, alpha: number, glintP: number) {
  ctx.save();
  ctx.globalAlpha = alpha;
  ctx.translate(x, y);
  ...
  ctx.shadowBlur = 14;
  ctx.shadowColor = "rgba(244, 96, 54, 0.6)";

  if (spriteReady(paddleSprite)) {
    const drawW = PADDLE_IMG_W * PADDLE_IMG_SCALE;
    const drawH = PADDLE_IMG_H * PADDLE_IMG_SCALE;
    const drawX = -PADDLE_NECK_X * PADDLE_IMG_SCALE;
    const drawY = -PADDLE_NECK_Y * PADDLE_IMG_SCALE;
    const faceH = PADDLE_NECK_Y * PADDLE_IMG_SCALE;
    ctx.drawImage(paddleSprite, drawX, drawY, drawW, drawH);

    if (glintP > 0) {
      ctx.save();
      ctx.beginPath();
      ctx.rect(drawX, drawY, drawW, faceH);
      ctx.clip();
      ctx.globalAlpha = alpha * Math.sin(glintP * Math.PI);
      const sweepX = drawX - 2 + glintP * (drawW + 12);
      const gg = ctx.createLinearGradient(sweepX, drawY, sweepX, drawY + faceH);
      gg.addColorStop(0, "rgba(255,255,255,0)");
      gg.addColorStop(0.5, "rgba(255,255,255,.95)");
      gg.addColorStop(1, "rgba(255,255,255,0)");
      ctx.fillStyle = gg;
      ctx.fillRect(sweepX, drawY, 4, faceH);
      ctx.restore();
    }
    ctx.restore();
    return;
  ...
  // Paddle face — rounded rectangle from y -40 (top) to y 4 (neck).
  const faceHalfW = 14,
    faceTop = -40,
    faceBottom = 4,
    corner = 10;
  const grd = ctx.createLinearGradient(-faceHalfW, faceTop, faceHalfW, faceBottom);
  ...
  ctx.lineWidth = 1.1;
  ctx.beginPath();
  ctx.moveTo(-faceHalfW, faceTop + corner);
  ctx.arcTo(-faceHalfW, faceTop, -faceHalfW + corner, faceTop, corner);
  ctx.lineTo(faceHalfW - corner, faceTop);
  ctx.arcTo(faceHalfW, faceTop, faceHalfW, faceTop + corner, corner);
  ctx.lineTo(faceHalfW, faceBottom - corner * 0.5);
  ctx.arcTo(faceHalfW, faceBottom, faceHalfW - corner * 0.5, faceBottom, corner * 0.5);
  ctx.lineTo(-faceHalfW + corner * 0.5, faceBottom);
  ctx.arcTo(-faceHalfW, faceBottom, -faceHalfW, faceBottom - corner * 0.5, corner * 0.5);
  ctx.closePath();
  ctx.fill();
  ctx.stroke();
  ...
  ctx.lineWidth = 0.8;
  ctx.beginPath();
  ctx.rect(-4, faceBottom, 8, 30);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(0, faceBottom + 30, 4.5, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ...
  if (glintP > 0) {
    ctx.save();
    ctx.globalAlpha = alpha * Math.sin(glintP * Math.PI);
    const sweepX = -faceHalfW - 2 + glintP * (faceHalfW * 2 + 12);
    const gg = ctx.createLinearGradient(sweepX, faceTop, sweepX, faceBottom);
    gg.addColorStop(0, "rgba(255,255,255,0)");
    gg.addColorStop(0.5, "rgba(255,255,255,.95)");
    gg.addColorStop(1, "rgba(255,255,255,0)");
    ctx.fillStyle = gg;
    ctx.fillRect(sweepX, faceTop, 4, faceBottom - faceTop);
    ctx.restore();
  }
  ctx.restore();
}
...
export default function PromoSuccessAnimation({
  ...
  useEffect(() => {
    ...
    // The real "Total to pay" span already shows the final discounted
    // number the instant the promo is applied (its re-render happens
    // before this component even mounts) — the overlay is what plays out
    // the reveal on top of it. Rather than parking an opaque box over it
    // (which shows up as a visible white rectangle against non-white
    // backgrounds), just hide the real element directly for the
    // overlay's lifetime; `visibility: hidden` keeps it in layout (so its
    // rect stays correct for measurement) without painting anything.
    // Restored the moment this effect tears down, i.e. exactly when the
    // overlay unmounts and there's nothing left masking it.
  ...
  useEffect(() => {
    ...
    // Nested function declarations below don't retain the narrowing above
    // (TS only narrows within the same closure scope), so pin it to a
    // definitely-non-null local for use inside startWithCanvas.
    const targetRectFinal = targetRect;

    // First mount of <ModalPortal> in this same commit: it starts
    // `mounted=false` and only flips true inside its own effect, so the
    // actual createPortal() insertion (and therefore canvasRef.current)
    // lands one commit *after* this effect runs. Bailing on the very
    // first check would silently kill the whole sequence before a frame
    // is ever drawn, so give it a couple of rAFs to show up first —
    // mirrors the targetRef poll above.
    let cancelled = false;
    let startRaf = 0;

    function startWithCanvas(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
    ...
    const tx = targetRectFinal.left + targetRectFinal.width / 2;
    const ty = targetRectFinal.top + targetRectFinal.height / 2;
    ...
    function frame(now: number) {
      ...
      if (t < FX.CHARGE_END) {
        ...
        drawCourtPulse(ctx!, sx, sy, p, cyan, "rgb(var(--color-orange))");
      ...
      if (t >= FX.CHARGE_END && t < FX.FLIGHT_END) {
        ...
        drawRallyTrajectory(ctx!, sx, sy, ix, iy, p, cyan, "rgb(var(--color-orange))");
        spawn(particles, ix, iy, { speed: rand(20, 60), maxLife: rand(0.22, 0.4), size: 1.6 });
        drawPickleball(ctx!, ix, iy, 15 * lerp(0.6, 1.15, Math.sin(p * Math.PI)), cyan);
      }

      // Phase 3 — paddle forms + glint pause
      let paddleRot = 32,
        paddleScale = 1,
        paddleAlpha = 0;
      if (t >= FX.FLIGHT_END && t < FX.DAGGER_PAUSE_END) {
        const p = clamp01((t - FX.FLIGHT_END) / (FX.DAGGER_PAUSE_END - FX.FLIGHT_END));
        paddleAlpha = clamp01(p * 4);
        paddleScale = lerp(1.3, 1, easeOutBack(clamp01(p * 2)));
        paddleRot = lerp(20, 34, p);
        drawPaddle(ctx!, paddleX, paddleY, paddleRot, paddleScale, paddleAlpha, p > 0.7 ? (p - 0.7) / 0.3 : 0);
        if (p > 0.7 && Math.random() < 0.4) {
          spawn(particles, paddleX, paddleY, { speed: rand(5, 18), maxLife: 0.18, size: 1 });
        }
      }

      // Phase 4 — smash + impact
      if (t >= FX.SLASH_START && t < FX.IMPACT_AT + 40) {
        ...
        for (let g = 0; g < 5; g++) {
          ...
          if (gp <= 0) continue;
          const gx = lerp(paddleX, tx, easeOutCubic(gp));
          const gy = lerp(paddleY, ty, easeOutCubic(gp));
          drawPaddle(ctx!, gx, gy, 55, 1.1, (1 - g / 5) * 0.5, 1);
        }
        const ex = lerp(paddleX, tx, easeOutCubic(p));
        const ey = lerp(paddleY, ty, easeOutCubic(p));
        drawPaddle(ctx!, ex, ey, 58, 1.15, 1, 1);

        if (t >= FX.IMPACT_AT && !impacted) {
          impacted = true;
          for (let i = 0; i < 54; i++) spawn(particles, tx, ty, { speed: rand(140, 430), maxLife: rand(0.3, 0.72), size: rand(1.8, 4.2) });
          drawCourtPulse(ctx!, tx, ty, 0.08, cyan, "rgb(var(--color-orange))");
          shakeScreen(240, 6);
          if (flashRef.current) {
    ...
    function pollForCanvas(tries: number) {
      if (cancelled) return;
      const canvas = canvasRef.current;
      const ctx = canvas?.getContext("2d") ?? null;
      if (canvas && ctx) {
        innerCleanup = startWithCanvas(canvas, ctx);
        return;
      }
      if (tries > 10) {
        console.error("[promoFx] canvasRef never attached (ModalPortal didn't mount in time) — aborting animation.");
        finish();
        return;
      }
      startRaf = requestAnimationFrame(() => pollForCanvas(tries + 1));
    }
    pollForCanvas(0);
    ...
    <ModalPortal lockScroll={false}>
      <div className="promofx-layer" aria-hidden="true" role="presentation">
        ...
        <div ref={stampRef} className="promofx-stamp-v3">
          ...
          <span className="promofx-stamp-text-v3">ACE<br />-{percentage}%</span>
        </div>
      </div>
    </ModalPortal>
  );
