...
type FAQ = { q: string; a: string; link?: string | null; linkLabel?: string | null };

type ApiFaq = { id: string; question: string; answer: string; link: string | null; linkLabel: string | null };
...
export default function FAQSection() {
  ...
  useEffect(() => {
    ...
    fetch("/api/faqs", { cache: "no-store" })
      ...
      .then((d) => {
        if (cancelled) return;
        const list: ApiFaq[] = Array.isArray(d.faqs) ? d.faqs : [];
        setFaqs(list.map((f) => ({ q: f.question, a: f.answer, link: f.link, linkLabel: f.linkLabel })));
      })
      .catch(() => {
  ...
  return (
    <div className="space-y-3">
      <AskAI faqCount={faqs?.length ?? null} />
      {faqs === null ? (
        <p className="text-sm text-white/50 px-1">Loading FAQs…</p>
      ) : faqs.length === 0 ? (
        <p className="text-sm text-white/50 px-1">No FAQs yet — check back soon.</p>
      ) : (
        faqs.map((item, i) => (
          <FAQItem
            key={item.q + i}
