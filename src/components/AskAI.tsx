...
const DEFAULT_TITLE = "Ask us anything";
const DEFAULT_SUBTITLE = "Instant answers, English or Bisaya — no waiting on Messenger.";
...
export default function AskAI({ faqCount }: { faqCount?: number | null }) {
  ...
  // Widget copy is admin-editable (Site content -> Homepage text), so pull
  // it the same lightweight way SiteHeader pulls branding — a small public
  // settings fetch, falling back to the defaults above if it's slow/fails.
  useEffect(() => {
    fetch("/api/site-settings", { cache: "no-store" })
      .then((r) => r.json())
      .then((d) => {
        if (d?.settings?.askAiTitle) setTitle(d.settings.askAiTitle);
        if (d?.settings?.askAiSubtitle) setSubtitle(d.settings.askAiSubtitle);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <div>
          <p className="font-display font-700 text-sm sm:text-base text-white">{title}</p>
          <p className="text-[11px] sm:text-xs text-white/60">{subtitle}</p>
        </div>
      </div>
