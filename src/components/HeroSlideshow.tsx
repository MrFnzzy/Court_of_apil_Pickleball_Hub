...
export default function HeroSlideshow({
  ...
  const goTo = (nextIndex: number) => {
    ...
    const forwardDistance = (nextIndex - current + count) % count;
    setDirection(forwardDistance <= count / 2 ? "forward" : "backward");
          ...
          <>
            {slides.map((slide, slideIndex) => {
              ...
              return (
                <div
                  key={slide.id}
                  className={`hero-slide absolute inset-0 ${isActive ? `hero-slide--active hero-slide--${direction}` : ""}`}
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
                {slides.map((slide, slideIndex) => (
                  <button
                    ...
                    className={`hero-slide__dot focus-ring ${slideIndex === index ? "hero-slide__dot--active" : ""}`}
                  />
                ))}
              </div>
            )}
