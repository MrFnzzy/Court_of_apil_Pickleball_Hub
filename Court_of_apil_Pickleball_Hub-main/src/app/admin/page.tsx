...
import ScheduleGrid, { type BookerInfo } from "@/components/ScheduleGrid";
...
export default function AdminDashboard() {
  ...
  // Hour → booker details, used by ScheduleGrid to show a tooltip on booked/pending slots
  // so the admin doesn't have to cross-reference the time against the list below.
  const bookingInfo: Record<number, BookerInfo> = {};
  for (const b of bookings) {
    if (b.status !== "PENDING" && b.status !== "CONFIRMED") continue;
    for (const hour of b.startHours) {
      bookingInfo[hour] = {
        name: b.customerName,
        contact: b.contactNumber,
        email: b.email,
        rentals: b.paddleCount,
      };
    }
  }
        ...
        {tab === "accounts" ? (
          ...
          <>
            <div className="rounded-court bg-white border-2 border-court-blue/20 shadow-court p-5 sm:p-6 mb-6">
              ...
              <div key={gridKey}>
                <ScheduleGrid date={date} mode="view" bookingInfo={bookingInfo} />
              </div>
            </div>
                  ...
                  <div key={b.id} className="rounded-court bg-white border-2 border-court-ink/10 shadow-court p-5">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-court-ink/50 mt-1">
                          Rentals: {b.paddleCount > 0 ? `${b.paddleCount} paddle${b.paddleCount > 1 ? "s" : ""}` : "None"}
                        </p>
                      </div>
