# Heide's Pickleball Hub — Booking Website
...
- **Automatic "thanks for playing" + feedback (`/feedback/[token]`)** — once a confirmed booking's reserved
  time has fully passed, the customer is automatically emailed a thank-you note with a link to a short
  built-in feedback form (star ratings for overall experience, venue/court quality, staff & service, and
  value for money, plus an optional comment). Submissions show up in the admin dashboard's **Feedback** tab
  with running averages. No cron job needed — it's checked lazily whenever bookings are loaded, same as the
  existing stale-pending-booking cleanup.
