...
- **Without `ANTHROPIC_API_KEY`** — it matches the question against your FAQ list using keyword overlap. Good
  enough for most phrasings, free, and has no external dependency. English only — it won't understand
  Bisaya/Cebuano or Tagalog questions.
- **With `ANTHROPIC_API_KEY`** — it calls Claude to understand rephrased or looser questions **in English,
  Tagalog, or Bisaya/Cebuano** (it replies in whatever language the visitor asked in), but is still strictly
  limited to answering from your FAQ list only (it's instructed to never invent policies, prices, or details)
  and falls back to the Facebook link for anything it isn't confident about. Since most of your customers will
  ask in Bisaya or Taglish, this is the version worth turning on.
