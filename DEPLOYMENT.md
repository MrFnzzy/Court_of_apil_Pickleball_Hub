...
No extra environment variable or signup is needed for the "read my receipt" auto-fill on the payment step — it runs on a free, open-source OCR library (Tesseract.js) directly on your server, with no external API or account involved. It's noticeably less accurate than a paid AI reading service would be (it can misread messy or low-quality screenshots, or grab the wrong number if a receipt has several), so treat what it fills in as a rough starting point — customers can and should double-check it before submitting. On Vercel's free Hobby plan, function calls are capped at 10 seconds, which occasionally isn't enough for a cold start; if a scan times out, the fields just stay blank for manual entry — nothing else breaks.
