...
| `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` | optional — enables invisible bot protection (reCAPTCHA v3) on the booking form. See "Bot protection" below for how to get one. |
...
The public booking form (`/book`) can require an invisible Google reCAPTCHA v3 check before a submission is
accepted — it scores how bot-like the request looked and rejects low scores, with **no checkbox or puzzle**
for real customers to deal with. Skipped entirely (every submission passes) until you configure it. To turn
it on:

1. Go to https://www.google.com/recaptcha/admin/create
2. Pick **reCAPTCHA v3**, add your site's domain (and `localhost` if you want it active in local dev too).
...
A small reCAPTCHA badge will appear in the bottom-right corner of the booking page once it's active — that's
required by Google, don't remove or hide it.
